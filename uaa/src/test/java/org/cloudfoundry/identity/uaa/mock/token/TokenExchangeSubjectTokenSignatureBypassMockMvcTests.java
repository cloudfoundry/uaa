...
class TokenExchangeSubjectTokenSignatureBypassMockMvcTests extends TokenExchangeMockMvcBase {
     ...
     *
     * <p>Only {@code getClaimsFromToken(String, IdentityProvider, boolean)} is overridden
     * (the overload the base class's {@code ExternalOAuthCodeToken} entry point actually
     * calls): it decodes the JWT payload directly without calling {@code validateToken()},
     * so any token — including one with a tampered payload — is accepted at the filter.
     * All other behaviour (user lookup, shadow-user creation, issuer-based IdP resolution)
    ...
    static class NoSignatureVerificationConfig {
        ...
        ExternalOAuthAuthenticationManager tokenExchangeAuthenticationManager(
            ...
            ExternalOAuthAuthenticationManager bean = new ExternalOAuthAuthenticationManager(
                ...
                Map<String, Object> getClaimsFromToken(String idToken, IdentityProvider<T> identityProvider, boolean enforceRelyingPartyAudience) {
