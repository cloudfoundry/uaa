...
class SamlMetadataEndpointMockMvcZonePathTests {
    ...
    void get_saml_metadata_within_zone(ZoneResolutionMode mode) throws Exception {
        ...
        MvcResult result = mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, "/saml/metadata"))
                .andExpect(status().isOk())
                .andReturn();
        assertThat(result.getResponse().getHeader(HttpHeaders.CONTENT_DISPOSITION)).contains("filename=\"saml-%s-sp.xml\";".formatted(subdomain));
    }
    ...
    void get_saml_metadata_with_trailing_slash_within_zone(ZoneResolutionMode mode) throws Exception {
        ...
        MvcResult result = mockMvc.perform(mode.createRequestBuilder(subdomain, HttpMethod.GET, path))
                .andExpect(status().isOk())
                .andReturn();
        assertThat(result.getResponse().getHeader(HttpHeaders.CONTENT_DISPOSITION)).contains("filename=\"saml-%s-sp.xml\";".formatted(subdomain));
    }
