...
public class MfaProviderEndpointsDocs extends InjectedMockContextTest {
    ...
    private static final String ID_DESC = "A GUID generated by the UAA to uniquely identify this MFA Provider.";
    private static final String CREATED_DESC = "Timestamp of MFA Provider creation.";
    private static final String LAST_MODIFIED_DESC = "Timestamp of last update to the MFA Provider.";
    ...
    private FieldDescriptor[] getGoogleMfaProviderFields() {
        return (FieldDescriptor[]) ArrayUtils.addAll(commonProviderFields, new FieldDescriptor[]{
                ...
                fieldWithPath("config.providerDescription").optional(null).type(STRING).description("A human-readable provider description."),
                fieldWithPath("config.digits").optional(6).type(NUMBER).description("Number of digits in the generated TOTP codes."),
                fieldWithPath("config.issuer").optional(null).type(STRING).description("Issuer. Defaults to Identity Zone name."),
                fieldWithPath("config.duration").optional(30).type(NUMBER).description("Length of time, in seconds, that each TOTP code will remain valid.")
        });
    }
