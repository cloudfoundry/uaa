...
class UserTokenMockMvcTests extends AbstractTokenMockMvcTests {
    ...
    void test_user_managed_token() throws Exception {
        ...
        UaaBaseClientDetails recipient = setUpClients(recipientId, "uaa.user", "uaa.user,test.scope", "password,"+GRANT_TYPE_REFRESH_TOKEN, true, TEST_REDIRECT_URI,
                Collections.singletonList("uaa"), 50000);

        String requestorId = "requestingClient"+new RandomValueStringGenerator().generate();
        UaaBaseClientDetails requestor = setUpClients(requestorId, "uaa.user", "uaa.user", "password,"+GRANT_TYPE_USER_TOKEN, true, TEST_REDIRECT_URI,
    ...
    void test_client_credentials_token() throws Exception {
        ...
        String requestorId = "requestingClient"+new RandomValueStringGenerator().generate();
        UaaBaseClientDetails requestor = setUpClients(requestorId, "uaa.user", "uaa.user", "client_credentials,"+GRANT_TYPE_USER_TOKEN, true, TEST_REDIRECT_URI,
                Collections.singletonList("uaa"));
    ...
    void test_invalid_grant_type() throws Exception {
        ...
        String requestorId = "requestingClient"+new RandomValueStringGenerator().generate();
        UaaBaseClientDetails requestor = setUpClients(requestorId, "uaa.user", "uaa.user", "password", true, TEST_REDIRECT_URI,
                Collections.singletonList("uaa"));
