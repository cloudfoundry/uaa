...
//        LDAP_BIND_PASSWORD,
    ...
    void createOidcIdentityProvider() throws Exception {
        ...
        FieldDescriptor[] idempotentFields = (FieldDescriptor[]) ArrayUtils.addAll(commonProviderFields, ArrayUtils.addAll(new FieldDescriptor[]{
                ...
                fieldWithPath("config.jwtClientAuthentication").optional(null).type(OBJECT).description("Only effective, if relyingPartySecret is not set or null. Uses private_key_jwt from OIDC or from OAuth2 (RFC 7523). Use OIDC if you set true and OAuth2 if you define sub elements iss and aud."),
                fieldWithPath("config.attributeMappings.user_name").optional("sub").type(STRING).description("Map `user_name` to the attribute for user name in the provider assertion or token. The default for OpenID Connect is `sub`."),
