...
class ClientAdminEndpointDocs extends AdminClientCreator {
    ...
    private static final FieldDescriptor[] clientJwtChangeFields = new FieldDescriptor[]{
            ...
            fieldWithPath("sub_pattern").constrained("Optional. Mutually exclusive with sub.").type(STRING).description("A subject pattern, matched against the whole sub claim of the retrieved OAuth2 compliant JWT. `*` matches one component of the subject, crossing neither the `:` claim separator nor `/`; `**` crosses `/` but never `:`, for a component such as a git ref that may contain a path. Every other character is matched literally. Intended for issuers that derive the subject from the execution context, e.g. `project_path:myteam/deploy:ref_type:branch:ref:**`. Must contain at least one wildcard and at least one entirely literal component, so `*` and `a*` are rejected; limited to 256 characters and five wildcards, and a subject longer than 256 characters is not matched. When deleting, the pattern is compared verbatim and does not remove the credentials it would match."),
