...
class ClientAdminEndpointDocs extends AdminClientCreator {
    ...
    private static final FieldDescriptor[] clientJwtChangeFields = new FieldDescriptor[]{
            ...
            fieldWithPath("sub_pattern").constrained("Optional. Mutually exclusive with sub.").type(STRING).description("A subject pattern, matched against the whole sub claim of the retrieved OAuth2 compliant JWT. `*` matches any characters except the `:` claim separator, so a wildcard cannot extend beyond the claim component it is written in; every other character is matched literally. Intended for issuers that derive the subject from the execution context, e.g. `project_path:myteam/deploy:ref_type:branch:ref:*`. Must contain at least one wildcard and some literal text, so `*` alone is rejected; limited to 256 characters and five wildcards, and a subject longer than 256 characters is not matched. When deleting, the pattern is compared verbatim and does not remove the credentials it would match."),
