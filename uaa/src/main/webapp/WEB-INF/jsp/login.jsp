...
<body id="micro">
    <div class="splash">
         ...
         <article class="container">
              <p>Sign in with Cloud Foundry credentials.</p>
              <form id="loginForm" name="loginForm"
                    action="<c:url value="/login.do"/>" method="POST" novalidate>
                <div>
                    <c:if test="${not empty param.error}">
                        <div class="flash">
                            Oops, we couldn't verify your email and password.
                        </div>
                    </c:if>
                    <c:forEach items="${prompts}" var="prompt">
                        <input id='${prompt.key}' type='${prompt.value[0]}'
                            name='${prompt.key}' placeholder='${prompt.key}' />
                    </c:forEach>

                </div>
                    <button type="submit" class="orange-button">Sign in</button>
                    <span class="button-alt">
                      <a href="http://${hostName}/signup">
                        Forget your password
                        <div class="question"><img src="/uaa/resources/images/icon-question.png"></div>
                      </a>
                    </span>
              </form>
         </article>
