...
public class UaaStringUtils {
  ...
  /**
   * Convert a string from camel case to underscores, also replacing periods with underscores (so
   * for example a fully qualified Java class name gets underscores everywhere).
   *
  ...
  public static Set<String> retainAllMatches(
      Collection<String> values, Collection<String> whitelist) {
    Set<Pattern> regExPatterns =
        UaaStringUtils.constructWildcards(
            new HashSet<>(whitelist),
            UaaStringUtils::constructSimpleWildcardPatternWithAnyCharDelimiter);
  ...
  /**
   * Escapes all regular expression patterns in a string so that when using the string itself in a
   * regular expression, only an exact literal match will return true. For example, the string ".*"
   * will not match any string, it will only match ".*". The value ".*" when escaped will be "\.\*"
   *
  ...
  /**
   * Escapes all regular expression patterns in a string so that when using the string itself in a
   * regular expression, only an exact literal match will return true.
   *
   * @param s - the string for which we need to escape regular expression constructs
  ...
  /**
   * Returns a pattern that does a single level regular expression match where the * character is a
   * wildcard until it encounters the next literal.
   *
   * @param s string
  ...
  public static Set<Pattern> constructWildcards(
      Collection<String> wildcardStrings, Function<String, String> replace) {
  ...
  public static Set<String> getStringsFromAuthorities(
  ...
  public static List<? extends GrantedAuthority> getAuthoritiesFromStrings(
