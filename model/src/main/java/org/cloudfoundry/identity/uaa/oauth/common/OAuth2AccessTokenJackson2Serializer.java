...
public class OAuth2AccessTokenJackson2Serializer extends StdSerializer<OAuth2AccessToken> {
    ...
    @Override
    public void serialize(OAuth2AccessToken token, JsonGenerator jgen, SerializationContext provider) {
        ...
        if (token instanceof CompositeToken compositeToken && compositeToken.getIdTokenValue() != null) {
            jgen.writeStringProperty(CompositeToken.ID_TOKEN, compositeToken.getIdTokenValue());
        }
