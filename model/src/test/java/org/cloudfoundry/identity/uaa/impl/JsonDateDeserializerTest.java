...
public class JsonDateDeserializerTest {
  ...
  public void testParsing() throws IOException, ParseException {
    ...
    Assert.assertEquals(
        new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'").parse(testDateString).getTime(),
        (long) d.getTime());
  }
      ...
      threadArray[i] =
          new Thread(
              () -> {
                try {
                  Date d =
                      JsonDateDeserializer.getDate(
                          testDateString, new JsonLocation(null, 22, 0, 0));
                  if (new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'")
