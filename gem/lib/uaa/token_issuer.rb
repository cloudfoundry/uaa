...
class TokenIssuer
  ...
  def implicit_grant_with_creds(credentials, scope = nil)
    ...
    begin
      ...
      raise BadResponse, "mismatched state" unless reply[:state] == state
      raise TargetError.new(reply), "error response from #{@target}" if reply[:error]
      raise BadResponse, "no type and token" unless reply[:token_type] && reply[:access_token]
    rescue URI::InvalidURIError, ArgumentError, BadResponse => e
      raise BadResponse, "bad response from #{@target}, #{e.message}"
    end
    Token.new reply
  end
  ...
  def implicit_grant(implicit_uri, callback_query)
    begin
      ...
      raise BadResponse, "mismatched state" unless params[:state] == in_params[:state]
      raise TargetError.new(reply), "error response from #{@target}" if reply[:error]
