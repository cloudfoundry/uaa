...
class ClientCli < BaseCli
  ...
  map "g" => "get"
  def get(name = Config.client_id)
    return help(__method__) if options[:help]
    Config.pp client_reg_request { |cr| cr.get(name) }
  end
  ...
  desc "update [name]", "update client registration info. alias 'cu'"
  ...
  def delete(name = nil)
    ...
    Config.pp client_reg_request { |cr| cr.delete(name) }
  end

  private

  def client_reg_request
    return yield CF::UAA::ClientReg.new(Config.target, auth_header, trace?)
