...
class LoginCli < BaseCli
  ...
  def refresh(rtoken = Config.opts[:refresh_token])
    return help(__method__) if options[:help]
    Config.opts user_token: issuer_request { |ti| ti.refresh_token_grant(rtoken).info }
  end
  ...
  def browser
    ...
    issuer_request(nil) do |ti|
      ...
      StubServer.responder do |request, reply|
        ...
        reply.headers[:content_type] = "text/plain"
        begin
          reply.body = "method: #{request.method}\r\npath: #{request.path}\r\nheaders: #{request.headers}\r\n\r\nbody: #{request.body}"
          #tokn = ti.implicit_grant(uri, URI.parse(request.path).query)
          #reply.body = "you are now logged in and can close this window"
        rescue TargetError => e
