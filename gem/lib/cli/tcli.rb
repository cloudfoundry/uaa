...
class TCli < BaseCli
  ...
  def target(uaa_url = nil, client_id = File.basename($0))
    ...
    if uaa_url && uaa_url.to_i.to_s == uaa_url
      Config.set_target_by_index uaa_url.to_i
    elsif uaa_url
      uaa_url = Config.normalize_url(uaa_url)
      begin
      ...
      Config.set_target uaa_url, client_id
    end
    puts Config.target ? "target set to: #{Config.target} #{Config.client_id}" : "no target set"
  end

  desc "settings", "display or set info for the current target"
  method_option :trace, type: :boolean, aliases: "-t", desc: "set default trace option"
