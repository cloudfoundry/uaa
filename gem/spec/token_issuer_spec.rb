...
describe Cloudfoundry::Uaa::TokenIssuer do
  ...
  it "should be able to get the prompts from the server", :integration=>true do
    @response = [200, '{"prompts":{"username":["text", "Username"], "password":["password", "Password"]}}', {'Content-Type' => "application/json"}]
  ...
  context "with client_credentials grant" do
    ...
    before :each do
      @response = [200, '{"access_token":"FOO"}', {'Content-Type' => "application/json"}]
    end
