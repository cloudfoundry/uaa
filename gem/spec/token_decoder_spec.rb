...
describe Cloudfoundry::Uaa::TokenDecoder do
  ...
  it "should raise an auth error if the given auth header is bad" do
    ...
    expect { subject.issuer_decode(nil) }.to raise_exception(Cloudfoundry::Uaa::TokenDecoder::AuthError)
    expect { subject.issuer_decode("one two three") }.to raise_exception(Cloudfoundry::Uaa::TokenDecoder::AuthError)
  end

  it "should raise a target error if the response is not json" do
    @stub_req.to_return(File.new(spec_asset('text_reply.txt')))
    expect { subject.issuer_decode("one two")}.should raise_exception(Cloudfoundry::Uaa::TokenDecoder::BadTarget)
  end
  ...
  it "should raise a target json error if the response is 400 with valid oauth json error" do
    ...
    expect { subject.issuer_decode("one two")}.should raise_exception(Cloudfoundry::Uaa::TokenDecoder::TargetJsonError)
  end

  it "should GET decoded token hash from the check_token endpoint" do
    @stub_req.to_return(File.new(spec_asset('check_token_success.txt')))
    info = subject.issuer_decode("one two")
