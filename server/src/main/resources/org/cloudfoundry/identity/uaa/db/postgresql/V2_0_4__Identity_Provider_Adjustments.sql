...
ALTER TABLE identity_provider ALTER COLUMN origin_key TYPE VARCHAR(36);
...
DROP INDEX IF EXISTS username_in_idp;

-- drop the column
ALTER TABLE users DROP COLUMN identity_provider_id;

-- unique is still username,origin,zone_id
CREATE UNIQUE INDEX users_unique_key ON users (origin,LOWER(username),identity_zone_id);

-- drop previous index
DROP INDEX IF EXISTS identity_provider_id;
