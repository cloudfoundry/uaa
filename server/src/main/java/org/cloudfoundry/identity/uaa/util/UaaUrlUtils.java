...
public abstract class UaaUrlUtils {
  ...
  private static final Pattern allowedRedirectUriPattern =
      Pattern.compile(
          "^([a-zA-Z][a-zA-Z0-9+\\*\\-.]*)://"
              +
              // URL starts with 'some-scheme://' or 'https://' or 'http*://
              "(.*:.*@)?"
  ...
  public static String getUaaUrl(
      String path, boolean zoneSwitchPossible, IdentityZone currentIdentityZone) {
   ...
   *
   * <ul>
   *   <li>If the requstedRedirectUri matches the whitelist the requestedRedirectUri is returned
   *   <li>If the whitelist is null or empty AND the fallbackRedirectUri is null, the
   *       requestedRedirectUri is returned - OPEN REDIRECT
   *   <li>If the whitelist is null or empty AND the fallbackRedirectUri is not null, the
   *       fallbackRedirectUri is returned
   * </ul>
   *
   * @param redirectUris - a whitelist collection of ant path patterns
   * @param requestedRedirectUri - the requested redirect URI, returned if whitelist matches or the
   *     fallbackRedirectUri is null
   * @param fallbackRedirectUri - returned if non null and the requestedRedirectUri doesn't match
   *     the whitelist redirectUris
   * @return a redirect URI, either the requested or fallback as described above
   */
  public static String findMatchingRedirectUri(
      Collection<String> redirectUris, String requestedRedirectUri, String fallbackRedirectUri) {
