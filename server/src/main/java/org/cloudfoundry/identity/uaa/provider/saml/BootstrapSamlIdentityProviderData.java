...
public class BootstrapSamlIdentityProviderData implements InitializingBean {
  ...
  private List<IdentityProviderWrapper<SamlIdentityProviderDefinition>> samlProviders =
      new LinkedList<>();
  private Map<String, Map<String, Object>> providers = null;

  public BootstrapSamlIdentityProviderData() {}

  public static IdentityProvider<SamlIdentityProviderDefinition> parseSamlProvider(
      SamlIdentityProviderDefinition def) {
  ...
  protected void parseIdentityProviderDefinitions() {
    if (getLegacyIdpMetaData() != null) {
      ...
      if (alias == null) {
        throw new IllegalArgumentException(
            "Invalid IDP - Alias must be not null for deprecated IDP.");
      }
    ...
    for (IdentityProviderWrapper wrapper : samlProviders) {
      String alias =
          getUniqueAlias((SamlIdentityProviderDefinition) wrapper.getProvider().getConfig());
  ...
  public void setIdentityProviders(Map<String, Map<String, Object>> providers) {
    ...
    for (Map.Entry entry : providers.entrySet()) {
      ...
      Boolean storeCustomAttributes =
          (Boolean) ((Map) entry.getValue()).get(STORE_CUSTOM_ATTRIBUTES_NAME);
      ...
      if (alias == null) {
        throw new IllegalArgumentException(
            "Invalid IDP - alias must not be null [" + metaDataLocation + "]");
      }
      if (metaDataLocation == null) {
        throw new IllegalArgumentException(
            "Invalid IDP - metaDataLocation must not be null [" + alias + "]");
      }
      def.setIdpEntityAlias(alias);
