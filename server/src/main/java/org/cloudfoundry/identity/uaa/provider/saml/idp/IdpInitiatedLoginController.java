...
public class IdpInitiatedLoginController {
    ...
    public void initiate(@RequestParam(value = "sp", required = false) String sp,
        ...
        if (!holder.get().getSamlServiceProvider().isActive()) {
            log.debug("SP[{}] is disabled, aborting saml response", UaaStringUtils.getCleanedUserControlString(sp));
        ...
        if (!holder.get().getSamlServiceProvider().getConfig().isEnableIdpInitiatedSso()) {
            log.debug("SP[{}] initiated login is disabled, aborting saml response", UaaStringUtils.getCleanedUserControlString(sp));
        ...
        try {
            ...
            log.debug("IDP is sending assertion for SP[{}] to {}", UaaStringUtils.getCleanedUserControlString(sp),
            ...
            log.debug("IDP initiated authentication and responded to SP[{}]", UaaStringUtils.getCleanedUserControlString(sp));
            ...
            log.debug("IDP is unable to process assertion for SP[{}]", UaaStringUtils.getCleanedUserControlString(sp), e);
