...
public abstract class TokenValidation {
  ...
  public static TokenValidation buildAccessTokenValidator(
      String tokenJwtValue, KeyInfoService keyInfoService) {
    AccessTokenValidation validator = new AccessTokenValidation(tokenJwtValue, keyInfoService);
    validator.checkSignature();
    return validator;
  }

  public static TokenValidation buildRefreshTokenValidator(
      String tokenJwtValue, KeyInfoService keyInfoService) {
    RefreshTokenValidation refreshTokenValidation =
        new RefreshTokenValidation(tokenJwtValue, keyInfoService);
    refreshTokenValidation.checkSignature();
    return refreshTokenValidation;
  }

  public static TokenValidation buildIdTokenValidator(
      String tokenJwtValue, SignatureVerifier verifier, KeyInfoService keyInfoService) {
  ...
  private SignatureVerifier fetchSignatureVerifierFromToken(Jwt tokenJwt) {
    ...
    if (signingKey == null) {
      throw new InvalidTokenException(
          String.format("Token header claim [kid] references unknown signing key : [%s]", kid));
    }
  ...
  public TokenValidation checkSignature(SignatureVerifier verifier) {
    try {
      ...
      throw new InvalidTokenException(
          "Could not verify token signature.", new InvalidSignatureException(token));
    }
    return this;
  }
  ...
  public TokenValidation checkIssuer(String issuer) {
    ...
    if (!equals(issuer, claims.get(ISS))) {
      throw new InvalidTokenException(
          "Invalid issuer (" + claims.get(ISS) + ") for token did not match expected: " + issuer,
          null);
    }
    return this;
  }
  ...
  protected TokenValidation checkRequestedScopesAreGranted(Collection<String> grantedScopes) {
    ...
    List<String> missingScopes =
        requestedScopes.stream()
            .filter(
                requestedScope ->
                    grantedScopePatterns.stream()
                        .noneMatch(
                            grantedScopePattern ->
                                grantedScopePattern.matcher(requestedScope).matches()))
            .collect(toList());
    if (!missingScopes.isEmpty()) {
      String scopeClaimKey = scopeClaimKey().keyName();
      String message =
          String.format(
              "Some required \"%s\" are missing: [%s]",
              scopeClaimKey, String.join(", ", missingScopes));
        ...
        checkClient(
            cid -> {
              if (!equals(cid, client.getClientId())) {
                throw new InvalidTokenException(
                    "Token's client ID does not match expected value: " + client.getClientId());
              }
              return client;
            });
    if (isUserToken(claims)) {
      return validation
          ...
          .checkRequiredUserGroups(
              ofNullable(
                      (Collection<String>)
                          client.getAdditionalInformation().get(REQUIRED_USER_GROUPS))
                  .orElse(emptySet()),
  ...
  protected TokenValidation checkRequiredUserGroups(
      Collection<String> requiredGroups, Collection<String> userGroups) {
    if (!UaaTokenUtils.hasRequiredUserGroups(requiredGroups, userGroups)) {
      throw new InvalidTokenException(
          "User does not meet the client's required group criteria.", null);
    }
    return this;
  }
  ...
  protected TokenValidation checkClient(Function<String, ClientDetails> getClient) {
    ...
    try {
      ...
      if (null == claims.get(USER_ID)) {
        ...
        clientScopes =
            ofNullable(client.getAuthorities())
                .map(a -> a.stream().map(GrantedAuthority::getAuthority).collect(toList()))
  ...
  public TokenValidation checkRevocationSignature(List<String> revocableSignatureList) {
    ...
    try {
      ...
      throw new InvalidTokenException(
          "Token bears an invalid or unparseable revocation signature.", ex);
    }
  ...
  protected TokenValidation checkAudience(Collection<String> clients) {
    ...
    if (audClaim instanceof String) {
      ...
      try {
        audience = ((List<?>) audClaim).stream().map(s -> (String) s).collect(toList());
      } catch (ClassCastException ex) {
        throw new InvalidTokenException(
            "The token's audience claim is invalid or unparseable.", ex);
      }
    }

    List<String> notInAudience =
        clients.stream().filter(c -> !audience.contains(c)).collect(toList());
    if (!notInAudience.isEmpty()) {
      String joinedAudiences =
          notInAudience.stream()
              .map(c -> "".equals(c) ? "EMPTY_VALUE" : c)
              .collect(Collectors.joining(", "));
      throw new InvalidTokenException(
          "Some parties were not in the token audience: " + joinedAudiences, null);
    }

    return this;
  }

  public TokenValidation checkRevocableTokenStore(
      RevocableTokenProvisioning revocableTokenProvisioning) {
    try {
      String tokenId;
      if (claims.containsKey(ClaimConstants.REVOCABLE)
          && (boolean) claims.get(ClaimConstants.REVOCABLE)) {
        ...
        try {
          revocableToken =
              revocableTokenProvisioning.retrieve(tokenId, IdentityZoneHolder.get().getId());
      ...
      throw new InvalidTokenException(
          "The token's revocability or JTI claim is invalid or unparseable.", ex);
    }
  ...
  private List<String> readScopesFromClaim(ScopeClaimKey scopeClaimKey) {
    ...
    InvalidTokenException unparsableClaimException =
        new InvalidTokenException(
            String.format("The token's \"%s\" claim is invalid or unparseable.", scopeKeyName));
