...
public class UserManagedAuthzApprovalHandler implements UserApprovalHandler {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(UserManagedAuthzApprovalHandler.class);
    ...
    BaseClientDetails client =
        (BaseClientDetails)
            clientDetailsService.retrieve(
                authorizationRequest.getClientId(), identityZoneManager.getCurrentIdentityZoneId());
    if (client != null && requestedScopes != null) {
      autoApprovedScopes.addAll(client.getAutoApproveScopes());
      autoApprovedScopes =
          UaaTokenUtils.retainAutoApprovedScopes(requestedScopes, autoApprovedScopes);
    }
    ...
    if (userApproval) {
      ...
      for (String approvalParameter : approvalParameters.keySet()) {
        if (approvalParameter.startsWith(SCOPE_PREFIX)) {
          approvedScopes.add(
              approvalParameters.get(approvalParameter).substring(SCOPE_PREFIX.length()));
