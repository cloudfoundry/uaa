...
public class AccountsController {
  ...
  public String sendActivationEmail(
      ...
      @RequestParam(value = "client_id", required = false) String clientId,
      @RequestParam(value = "redirect_uri", required = false) String redirectUri,
      @Valid @ModelAttribute("email") ValidEmail email,
    ...
    List<IdentityProvider> identityProviderList =
        DomainFilter.getIdpsForEmailDomain(
            identityProviderProvisioning.retrieveAll(true, IdentityZoneHolder.get().getId()),
            email.getEmail());
    identityProviderList =
        identityProviderList.stream()
    ...
    if (!validation.valid()) {
      return handleUnprocessableEntity(
          model, response, "error_message_code", validation.getMessageCode());
    }
    try {
      ...
      return handleUnprocessableEntity(
          model, response, "error_message", e.getMessagesAsOneString());
    }
