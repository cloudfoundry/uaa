...
public class ScimGroupBootstrap implements InitializingBean {
  ...
  private final MapCollector<String, String, String> collector =
      new MapCollector<>(
          g -> g,
          g ->
              (String)
                  getMessageSource().getProperty(String.format(messagePropertyNameTemplate, g)));

  public ScimGroupBootstrap(
  ...
  public void setDefaultUserGroups(Set<String> defaultUserGroups) {
    ...
    this.defaultUserGroups = defaultUserGroups.stream().collect(collector);
  ...
  public void setNonDefaultUserGroups(Set<String> nonDefaultUserGroups) {
    ...
    this.nonDefaultUserGroups = nonDefaultUserGroups.stream().collect(collector);
  ...
  private void setCombinedGroups() {
    ...
    this.groups =
        this.groups.entrySet().stream()
            .collect(
                new MapCollector<>(
                    e -> StringUtils.trimWhitespace(e.getKey()),
                    e -> StringUtils.trimWhitespace(e.getValue())));
  }

  /**
   * Specify the membership info as a list of strings, where each string takes the format - {@code
   * <group-name>|<comma-separated usernames of members>[|write]} the optional 'write' field in the
   * end marks the users as admins of the group
   */
  ...
  public void afterPropertiesSet() {
    ...
    for (int i = 0; i < groupInfos.size(); i++) {
      ...
      if (StringUtils.hasText(description)) {
        ...
        try {
          groupInfos.set(
              i, scimGroupProvisioning.update(g.getId(), g, IdentityZoneHolder.get().getId()));
    ...
    for (String name : names) {
      ...
      if (member != null) {
        members.add(
            new ScimGroupMember(
                member.getId(),
                (member instanceof ScimGroup)
                    ? ScimGroupMember.Type.GROUP
  ...
  private ScimUser getUser(String name) {
    List<ScimUser> user =
        scimUserProvisioning.query(
            String.format(USER_BY_NAME_FILTER, name), IdentityZoneHolder.get().getId());
  ...
  ScimGroup getGroup(String name) {
    try {
      ...
      gr.setMembers(
          membershipManager.getMembers(gr.getId(), false, IdentityZoneHolder.get().getId()));
      return gr;
