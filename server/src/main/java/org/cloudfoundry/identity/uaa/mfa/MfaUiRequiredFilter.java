...
public class MfaUiRequiredFilter extends GenericFilterBean {
    ...
    switch (getNextStep(request)) {
      case INVALID_AUTH:
        ...
        response.sendError(
            ...
            "Invalid authentication object for UI operations.");
        break;
      ...
      case MFA_REQUIRED:
        logger.debug(
            "Request requires MFA, redirecting to MFA flow for " + getAuthenticationLogInfo());
  ...
  protected String getAuthenticationLogInfo() {
    ...
    if (a instanceof UaaAuthentication) {
      ...
      result.append("Unknown Auth=").append(a).append(" Principal=").append(a.getPrincipal());
    }
  ...
  protected MfaNextStep getNextStep(HttpServletRequest request) {
    ...
    if (!inProgressMatcher.matches(request)
        && !uaaAuth.getAuthenticationMethods().contains("mfa")) {
  ...
  protected void sendRedirect(
      ...
      throws IOException {
    StringBuilder url =
        new StringBuilder(redirectUrl.startsWith("/") ? request.getContextPath() : "");
  ...
  protected boolean mfaRequired(String origin) {
    return checker.isMfaEnabled(IdentityZoneHolder.get())
        && checker.isRequired(IdentityZoneHolder.get(), origin);
  }
