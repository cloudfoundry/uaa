...
public class AuthorizationAttributesParser {
  ...
  /**
   * This method searches the authorities in the request for additionalAuthorizationAttributes and
   * returns a map of these attributes that will later be added to the token
   */
    ...
    if (StringUtils.hasLength(authoritiesJson)) {
      try {
        Map<String, Object> authorities =
            JsonUtils.readValue(authoritiesJson, new TypeReference<>() {});
