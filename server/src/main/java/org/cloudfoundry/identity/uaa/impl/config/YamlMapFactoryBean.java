 ...
 * configuration, and it has some useful hierarchical properties. It's more or less a superset of
 * JSON, so it has a lot of similar features. If multiple resources are provided the later ones will
 * override entries in the earlier ones hierarchically - that is all entries with the same nested
 * key of type Map at any depth are merged. For example:
 *
 * <pre>
 * foo:
 *   bar:
 ...
 * second, but it's nested values are merged.
 *
 * @author Dave Syer
 */
