...
public final class ScimUtils {
  ...
  public static ExpiringCode getExpiringCode(
    ...
    return codeStore.generateCode(codeDataString, expiresAt, intent.name(), currentZoneId);
  }
  ...
  public static URL getVerificationURL(
      ExpiringCode expiringCode, IdentityZone currentIdentityZone) {
    String url = "";
    try {
      ...
      logger.error(
          String.format("Unexpected error creating user verification URL from %s", url), mfue);
    }
