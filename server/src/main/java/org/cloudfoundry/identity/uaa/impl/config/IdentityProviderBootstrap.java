...
public class IdentityProviderBootstrap
  ...
  public void validateDuplicateAlias(String originKey) {
    for (IdentityProvider provider :
        providers.stream().map(IdentityProviderWrapper::getProvider).collect(toList())) {
  ...
  protected void addLdapProvider() {
    ...
    */
    boolean override =
        ldapConfig == null || ldapConfig.get("override") == null
            ? true
            : (boolean) ldapConfig.get("override");
    if (!override) {
      IdentityProvider existing =
          getProviderByOriginIgnoreActiveFlag(LDAP, IdentityZone.getUaaZoneId());
  ...
  protected LdapIdentityProviderDefinition getLdapConfigAsDefinition(
  ...
  protected void populateLdapEnvironment(Map<String, Object> ldapConfig) {
    // this method reads the environment and overwrites values (needed by LdapMockMvcTests that
    // overrides properties through env)
    ...
    // but we can also have string properties like
    // ldap.attributeMappings.user.attribute.mapToAttributeName=mapFromAttributeName
  ...
  protected void addKeystoneProvider() {
    boolean keystoneProfile =
        Arrays.asList(environment.getActiveProfiles()).contains(OriginKeys.KEYSTONE);
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    for (IdentityProviderWrapper wrapper : providers) {
      ...
      IdentityProvider existing =
          getProviderByOriginIgnoreActiveFlag(provider.getOriginKey(), zoneId);
  ...
  private void deleteIdentityProviders(String zoneId) {
    for (String origin : getOriginsToDelete()) {
      if (!UAA.equals(origin) && !LDAP.equals(origin)) {
        ...
        // delete provider
        if (provider != null) {
          EntityDeletedEvent<IdentityProvider> event =
              new EntityDeletedEvent<>(
                  provider, SYSTEM_AUTHENTICATION, IdentityZoneHolder.getCurrentZoneId());
          if (this.publisher != null) {
            ...
            logger.warn(
                String.format(
                    "Unable to delete identity provider with origin '%s', no application publisher",
                    origin));
          }
        }
      }
  ...
  protected void updateDefaultZoneUaaIDP() {
    String zoneId = IdentityZone.getUaaZoneId();
    IdentityProvider internalIDP =
        getProviderByOriginIgnoreActiveFlag(UAA, IdentityZone.getUaaZoneId());
