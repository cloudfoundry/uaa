...
public class JdbcRevocableTokenProvisioning implements RevocableTokenProvisioning, SystemDeletable {
  ...
  private static final String FIELDS =
      "token_id,client_id,user_id,format,response_type,issued_at,expires_at,scope,data,identity_zone_id";
  private static final String UPDATE_FIELDS =
      FIELDS.substring(FIELDS.indexOf(',') + 1, FIELDS.lastIndexOf(',')).replace(",", "=?,") + "=?";
  private static final String TABLE = "revocable_tokens";
  private static final String GET_QUERY =
      "SELECT " + FIELDS + " FROM " + TABLE + " WHERE token_id=? AND identity_zone_id=?";
  private static final String GET_BY_USER_QUERY =
      "SELECT " + FIELDS + " FROM " + TABLE + " WHERE user_id=? AND identity_zone_id=?";
  private static final String GET_BY_CLIENT_QUERY =
      "SELECT " + FIELDS + " FROM " + TABLE + " WHERE client_id=? AND identity_zone_id=?";
  private static final String UPDATE_QUERY =
      "UPDATE " + TABLE + " SET " + UPDATE_FIELDS + " WHERE token_id=? and identity_zone_id=?";
  private static final String INSERT_QUERY =
      "INSERT INTO " + TABLE + " (" + FIELDS + ") VALUES (?,?,?,?,?,?,?,?,?,?)";
  private static final String DELETE_QUERY =
      "DELETE FROM " + TABLE + " WHERE token_id=? and identity_zone_id=?";
  private static final String DELETE_REFRESH_TOKEN_QUERY =
      "DELETE FROM "
          + TABLE
          + " WHERE user_id=? AND client_id=? AND response_type='"
          + REFRESH_TOKEN_RESPONSE_TYPE
          + "' AND identity_zone_id=?";
  private static final String DELETE_BY_CLIENT_QUERY =
      "DELETE FROM " + TABLE + " WHERE client_id = ? AND identity_zone_id=?";
  private static final String DELETE_BY_USER_QUERY =
      "DELETE FROM " + TABLE + " WHERE user_id = ? AND identity_zone_id=?";
  private static final String DELETE_BY_ZONE_QUERY =
      "DELETE FROM " + TABLE + " WHERE identity_zone_id=?";

  private static final Logger logger =
      LoggerFactory.getLogger(JdbcRevocableTokenProvisioning.class);
  ...
  public JdbcRevocableTokenProvisioning(
      JdbcTemplate jdbcTemplate, LimitSqlAdapter limitSqlAdapter, TimeService timeService) {
  ...
  private void checkExpired() {
    ...
    if ((now - lastCheck) > EXPIRATION_CHECK_INTERVAL.toMillis()
        && lastExpiredCheck.compareAndSet(lastCheck, now)) {
  ...
  /**
   * @return true if the last delete action deleted the max rows, this also signals there could be
   *     more rows to be deleted.
   */
  private boolean runDeleteExpired(long now) {
    final int maxRows = 500;
    String sql = limitSqlAdapter.getDeleteExpiredQuery(TABLE, "token_id", "expires_at", maxRows);
    int removed;
    do {
      removed = template.update(sql, now);
      logger.info("Removed " + removed + " expired revocable tokens.");
    } while (removed > 0
        && (timeService.getCurrentTimeMillis() - now) < maxExpirationRuntime.toMillis());
