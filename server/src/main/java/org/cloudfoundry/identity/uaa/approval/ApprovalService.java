...
public class ApprovalService {
  ...
  public void ensureRequiredApprovals(
      String userId,
      Collection<String> requestedScopes,
      String grantType,
      BaseClientDetails clientDetails) {
    Set<String> autoApprovedScopes =
        getAutoApprovedScopes(grantType, requestedScopes, clientDetails.getAutoApproveScopes());
    ...
    List<Approval> approvals =
        approvalStore.getApprovals(
            userId, clientDetails.getClientId(), IdentityZoneHolder.get().getId());
    for (Approval approval : approvals) {
      if (requestedScopes.contains(approval.getScope())
          && approval.getStatus() == Approval.ApprovalStatus.APPROVED) {
    ...
    if (!approvedScopes.containsAll(requestedScopes)) {
      logger.debug(
          "All requested scopes "
              + requestedScopes
              + " were not approved. Approved scopes: "
              + approvedScopes);
  ...
  private Set<String> getAutoApprovedScopes(
      Object grantType, Collection<String> tokenScopes, Set<String> autoapprovedScopes) {
