...
public class IdentityZoneHolder {
  ...
  private static final ThreadLocal<KeyManager> KEY_MANAGER_THREAD_LOCAL =
  ...
  private static final ThreadLocal<IdentityZone> IDENTITY_ZONE_THREAD_LOCAL =
      InheritableThreadLocal.withInitial(() -> getUaaZone(provisioning));
  ...
  public static KeyManager getSamlSPKeyManager() {
    ...
    keyManager =
        samlKeyManagerFactory.getKeyManager(
            IDENTITY_ZONE_THREAD_LOCAL.get().getConfig().getSamlConfig());
    if (keyManager != null) {
    ...
    keyManager =
        samlKeyManagerFactory.getKeyManager(getUaaZone(provisioning).getConfig().getSamlConfig());
    KEY_MANAGER_THREAD_LOCAL.set(keyManager);
    return keyManager;
  }
