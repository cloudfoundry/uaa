...
public class ZoneEndpointsClientDetailsValidator implements ClientDetailsValidator {
  ...
  public ZoneEndpointsClientDetailsValidator(final ClientSecretValidator clientSecretValidator) {
  ...
  public ClientDetails validate(ClientDetails clientDetails, Mode mode)
    ...
    if (mode == Mode.CREATE) {
      ...
      if (!Collections.singleton("uaa.resource")
          .equals(AuthorityUtils.authorityListToSet(clientDetails.getAuthorities()))) {
      ...
      if (!Collections.singletonList(OriginKeys.UAA)
          .equals(
      ...
      if (!REQUIRED_SCOPE.equals(
          clientDetails.getAdditionalInformation().get(ClientConstants.CREATED_WITH))) {
        throw new InvalidClientDetailsException(
            "client must have been " + ClientConstants.CREATED_WITH + " scope " + REQUIRED_SCOPE);
      }
