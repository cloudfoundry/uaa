...
public class PasswordChangeEventPublisher implements ApplicationEventPublisherAware {
  ...
  public PasswordChangeEventPublisher(
      ScimUserProvisioning provisioning, IdentityZoneManager identityZoneManager) {
  ...
  public void passwordFailure(String userId, Exception e) {
    ...
    publish(
        new PasswordChangeFailureEvent(
            e.getMessage(), user, getPrincipal(), identityZoneManager.getCurrentIdentityZoneId()));
  }
  ...
  String getEmail(ScimUser scimUser) {
    ...
    if (emails == null || emails.isEmpty()) {
      return scimUser.getUserName().contains("@")
          ? scimUser.getUserName()
          : scimUser.getUserName() + "@" + DEFAULT_EMAIL_DOMAIN;
    }
