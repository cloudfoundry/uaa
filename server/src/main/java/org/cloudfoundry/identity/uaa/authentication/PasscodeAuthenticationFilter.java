...
/**
 * Authentication filter to verify one time passwords with what's cached in the one time password
 * store.
 */
public class PasscodeAuthenticationFilter
  ...
  public PasscodeAuthenticationFilter(
        ...
        if (value.startsWith("{")) {
          try {
            Map<String, String> jsonCredentials =
                JsonUtils.readValue(value, new TypeReference<Map<String, String>>() {});
  ...
  protected static class ExpiringCodeAuthenticationManager implements AuthenticationManager {
    ...
    public ExpiringCodeAuthenticationManager(
        UaaUserDatabase uaaUserDatabase,
    ...
    public Authentication authenticate(Authentication authentication)
      ...
      if (!(authentication instanceof PasscodeAuthenticationFilter.ExpiringCodeAuthentication)) {
        ...
        PasscodeAuthenticationFilter.ExpiringCodeAuthentication expiringCodeAuthentication =
            (PasscodeAuthenticationFilter.ExpiringCodeAuthentication) authentication;
        // Validate passcode
        logger.debug("Located credentials in request, with passcode");
        if (methods != null
            && !methods.contains(
                expiringCodeAuthentication.getRequest().getMethod().toUpperCase())) {
        ...
        if (eCode != null && eCode.getData() != null) {
          try {
            ...
            throw new InsufficientAuthenticationException(
                "Unable to deserialize passcode object.", e);
          }
        }
        ...
        if (null != pi.getAuthorizationParameters()) {
          externalAuthorities =
              (Collection<GrantedAuthority>) pi.getAuthorizationParameters().get("authorities");
        }
        ...
        Authentication result =
            new UsernamePasswordAuthenticationToken(
                principal,
                null,
                externalAuthorities == null || externalAuthorities.size() == 0
                    ? authorities
                    : externalAuthorities);

        // add additional parameters for backwards compatibility
        PasscodeHttpServletRequest pcRequest =
            (PasscodeHttpServletRequest) expiringCodeAuthentication.getRequest();
