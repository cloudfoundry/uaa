...
public class ExternalLoginAuthenticationManager<ExternalAuthenticationDetails>
  ...
  public Authentication authenticate(Authentication request) throws AuthenticationException {
    ...
    try {
      logger.debug(
          String.format(
              "Searching for user by (username:%s , origin:%s)",
      ...
      logger.debug(
          String.format(
              "Searching for user by (email:%s , origin:%s)",
              userFromRequest.getEmail(), getOrigin()));
    ...
    UaaAuthentication success =
        new UaaAuthentication(
            new UaaPrincipal(user), user.getAuthorities(), uaaAuthenticationDetails);
    populateAuthenticationAttributes(success, request, authenticationData);
    publish(
        new IdentityProviderAuthenticationSuccessEvent(
            user, success, user.getOrigin(), IdentityZoneHolder.getCurrentZoneId()));
    return success;
  }
  ...
  protected void populateAuthenticationAttributes(
      ...
      ExternalAuthenticationDetails authenticationData) {
    ...
    if ((hasUserAttributes(authentication) || hasExternalGroups(authentication))
        && getProviderProvisioning() != null) {
      IdentityProvider<ExternalIdentityProviderDefinition> provider =
      ...
      if (provider.getConfig() != null && provider.getConfig().isStoreCustomAttributes()) {
        logger.debug(
            "Storing custom attributes for user_id:" + authentication.getPrincipal().getId());
        UserInfo userInfo =
            new UserInfo()
                .setUserAttributes(authentication.getUserAttributes())
                .setRoles(
                    new LinkedList(
                        ofNullable(authentication.getExternalGroups()).orElse(EMPTY_SET)));
  ...
  private boolean hasExternalGroups(UaaAuthentication authentication) {
    return authentication.getExternalGroups() != null
        && !authentication.getExternalGroups().isEmpty();
  }

  private boolean hasUserAttributes(UaaAuthentication authentication) {
    return authentication.getUserAttributes() != null
        && !authentication.getUserAttributes().isEmpty();
  }

  protected ExternalAuthenticationDetails getExternalAuthenticationDetails(
  ...
  protected UaaUser getUser(Authentication request, ExternalAuthenticationDetails authDetails) {
    ...
    if (request.getPrincipal() instanceof UserDetails) {
      ...
      userDetails =
          new User(username, password, true, true, true, true, UaaAuthority.USER_AUTHORITIES);
    ...
    String phoneNumber =
        (userDetails instanceof DialableByPhone)
            ? ((DialableByPhone) userDetails).getPhoneNumber()
            : null;
    String externalId =
        (userDetails instanceof ExternallyIdentifiable)
            ? ((ExternallyIdentifiable) userDetails).getExternalId()
            : name;
    boolean verified =
        (userDetails instanceof VerifiableUser)
            ? ((VerifiableUser) userDetails).isVerified()
            : false;
    UaaUserPrototype userPrototype =
  ...
  protected boolean haveUserAttributesChanged(UaaUser existingUser, UaaUser user) {
    if (!StringUtils.equals(existingUser.getGivenName(), user.getGivenName())
  ...
  protected List<? extends GrantedAuthority> mapAuthorities(
    ...
    for (GrantedAuthority authority : authorities) {
      ...
      for (ScimGroupExternalMember internalGroup :
          externalMembershipManager.getExternalGroupMapsByExternalGroup(
              externalGroup, origin, IdentityZoneHolder.get().getId())) {
