...
public class JdbcApprovalStore
    implements ApprovalStore, ApplicationEventPublisherAware, SystemDeletable {

  public static final String DELETE_OF_USER_APPROVALS_BY_PROVIDER =
      "delete from authz_approvals where user_id in (select id from users where origin = ? and identity_zone_id = ?)";
  protected static final String DELETE_ZONE_APPROVALS =
      "delete from authz_approvals where identity_zone_id = ?";
  protected static final String DELETE_CLIENT_APPROVALS =
      "delete from authz_approvals where client_id = ? and identity_zone_id = ?";
  protected static final String DELETE_USER_APPROVALS =
      "delete from authz_approvals where user_id = ? and identity_zone_id = ?";
  ...
  private static final String REFRESH_AUTHZ_SQL =
      String.format(
          "update %s set lastModifiedAt=?, expiresAt=?, status=? where user_id=? and client_Id=? and scope=? and identity_zone_id=?",
          TABLE_NAME);
  private static final String GET_AUTHZ_SQL =
      String.format("select %s from %s", FIELDS, TABLE_NAME);
  private static final String DELETE_AUTHZ_SQL = String.format("delete from %s", TABLE_NAME);
  private static final String EXPIRE_AUTHZ_SQL =
      String.format("update %s set expiresAt = :expiry", TABLE_NAME);
    ...
    int refreshed =
        jdbcTemplate.update(
            ...
            ps -> {
              ...
              ps.setString(
                  3, (approval.getStatus() == null ? APPROVED : approval.getStatus()).toString());
      ...
      int count =
          jdbcTemplate.update(
              ...
              ps -> {
                ...
                ps.setString(
                    5, (approval.getStatus() == null ? APPROVED : approval.getStatus()).toString());
  ...
  @Override
  public boolean revokeApprovalsForClientAndUser(
      String clientId, String userId, final String zoneId) {
    String sql = handleRevocationsAsExpiry ? EXPIRE_AUTHZ_SQL : DELETE_AUTHZ_SQL;
  ...
  public boolean purgeExpiredApprovals() {
    ...
    try {
      int deleted =
          jdbcTemplate.update(
              DELETE_AUTHZ_SQL + " where expiresAt <= ?",
  ...
  public int deleteByIdentityZone(String zoneId) {
    ...
    getLogger()
        .debug(String.format("Deleted zone approvals '%s' and count:%s", zoneId, approvalCount));
  ...
  public int deleteByOrigin(String origin, String zoneId) {
    ...
    getLogger()
        .debug(
            String.format(
                "Deleted provider approvals '%s'/%s and count:%s", origin, zoneId, approvalCount));
    return approvalCount;
  }
  ...
  public int deleteByClient(String clientId, String zoneId) {
    ...
    getLogger()
        .debug(String.format("Deleted client '%s' and %s approvals", clientId, approvalCount));
    return approvalCount;
  }
