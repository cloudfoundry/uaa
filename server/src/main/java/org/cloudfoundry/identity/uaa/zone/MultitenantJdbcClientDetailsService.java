...
public class MultitenantJdbcClientDetailsService extends MultitenantClientServices
    implements ResourceMonitor<ClientDetails>, SystemDeletable {

  protected static final Logger logger =
      LoggerFactory.getLogger(MultitenantJdbcClientDetailsService.class);
    ...
    try {
      details =
          jdbcTemplate.queryForObject(
              selectClientDetailsSql, new ClientDetailsRowMapper(), clientId, zoneId);
  ...
  public void addClientDetails(ClientDetails clientDetails, String zoneId)
    ...
    try {
      jdbcTemplate.update(
          DEFAULT_INSERT_STATEMENT, getInsertClientDetailsFields(clientDetails, zoneId));
  ...
  public void updateClientDetails(ClientDetails clientDetails, String zoneId)
    ...
    int count =
        jdbcTemplate.update(DEFAULT_UPDATE_STATEMENT, getFieldsForUpdate(clientDetails, zoneId));
    if (count != 1) {
  ...
  public void updateClientSecret(String clientId, String secret, String zoneId)
    ...
    int count =
        jdbcTemplate.update(
            DEFAULT_UPDATE_SECRET_STATEMENT, passwordEncoder.encode(secret), clientId, zoneId);
    if (count != 1) {
  ...
  public List<ClientDetails> listClientDetails(String zoneId) {
    return listFactory.getList(
        DEFAULT_FIND_STATEMENT, Collections.singletonMap("identityZoneId", zoneId), rowMapper);
  }
  ...
  private Object[] getFieldsForUpdate(ClientDetails clientDetails, String zoneId) {
    ...
    Map<String, Object> additionalInformation =
        new HashMap(clientDetails.getAdditionalInformation());
    Collection<String> requiredGroups =
        (Collection<String>) additionalInformation.remove(REQUIRED_USER_GROUPS);
  ...
  public void addClientSecret(String clientId, String newSecret, String zoneId)
    ...
    StringBuilder newSecretBuilder =
        new StringBuilder()
            .append(
                clientDetails.getClientSecret() == null
                    ? ""
    ...
    int count =
        jdbcTemplate.update(
            DEFAULT_UPDATE_SECRET_STATEMENT, newSecretBuilder.toString(), clientId, zoneId);
    if (count != 1) {
      throw new NoSuchClientException("No client found with id = " + clientId);
    }
  }
  ...
  public void deleteClientSecret(String clientId, String zoneId) throws NoSuchClientException {
    ...
    int count =
        jdbcTemplate.update(DEFAULT_UPDATE_SECRET_STATEMENT, clientSecret, clientId, zoneId);
    if (count != 1) {
  ...
  public int getTotalCount() {
    Integer count =
        jdbcTemplate.queryForObject("select count(*) from oauth_client_details", Integer.class);
    if (count != null) {
      return count;
    }
    return 0;
  ...
  protected String getUserId() {
    ...
    if (authentication.getPrincipal() instanceof UaaPrincipal) {
      ...
      ContextSensitiveOAuth2SecurityExpressionMethods
          contextSensitiveOAuth2SecurityExpressionMethods =
              new ContextSensitiveOAuth2SecurityExpressionMethods(authentication);
      userId =
          getCreatedByForClientAndZone(
              (String) authentication.getPrincipal(),
              contextSensitiveOAuth2SecurityExpressionMethods.getAuthenticationZoneId());
    }
    return userId;
  }
  ...
  String getCreatedByForClientAndZone(String clientId, String zoneId) {
    return jdbcTemplate.queryForObject(
        GET_CREATED_BY_SQL, new Object[] {clientId, zoneId}, String.class);
  }
    ...
    public ClientDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      if (json != null) {
        try {
          ...
          Object autoApprovedFromAddInfo =
              additionalInformation.remove(ClientConstants.AUTO_APPROVE);
          details.setAdditionalInformation(additionalInformation);
          if (autoApprovedFromAddInfo != null) {
            if ((autoApprovedFromAddInfo instanceof Boolean && (Boolean) autoApprovedFromAddInfo
                || "true".equals(autoApprovedFromAddInfo))) {
              autoApproveScopes.add("true");
            } else if (autoApprovedFromAddInfo instanceof Collection<?>) {
              @SuppressWarnings("unchecked")
              Collection<? extends String> approvedScopes =
                  (Collection<? extends String>) autoApprovedFromAddInfo;
      ...
      if (StringUtils.isEmpty(requiredUserGroups)) {
        ...
        details.addAdditionalInformation(
            REQUIRED_USER_GROUPS, commaDelimitedListToSet(requiredUserGroups));
      }
