...
public class SimpleSearchQueryConverter implements SearchQueryConverter {
  ...
  // LOWER
  private static final List<String> VALID_ATTRIBUTE_NAMES =
      List.of(
          "id",
          "created",
          "lastmodified",
  ...
  private String getWhereClause(
    ...
    try {
      final SCIMFilter zoneIdFilter =
          SCIMFilter.createEqualityFilter(AttributePath.parse("identity_zone_id"), zoneId);
      ...
      throw new IllegalArgumentException(
          "Invalid SCIM Filter: " + filter + "; Message: " + e.getMessage());
    }
  }

  @Override
  public MultiValueMap<String, Object> getFilterValues(String filter, List<String> validAttributes)
  ...
  private void validateFilterAttributes(SCIMFilter filter, List<String> validAttributeNames)
    ...
    if (!invalidAttributes.isEmpty()) {
      throw new InvalidResourceException(
          "Invalid filter attributes:"
              + StringUtils.collectionToCommaDelimitedString(invalidAttributes));
    }
  }

  private void validateFilterAttributes(
      SCIMFilter filter, List<String> invalidAttribues, List<String> validAttributeNames) {
    if (filter.getFilterAttribute() != null
        && filter.getFilterAttribute().getAttributeName() != null) {
  ...
  private void extractValues(SCIMFilter filter, MultiValueMap<String, Object> values)
      throws SCIMException {
      ...
      case AND:
        return "("
            + whereClauseFromFilter(
                filter.getFilterComponents().get(0), values, mapper, paramPrefix)
            + " AND "
            + whereClauseFromFilter(
                filter.getFilterComponents().get(1), values, mapper, paramPrefix)
            + ")";
      case OR:
        return "("
            + whereClauseFromFilter(
  ...
  private String comparisonClause(
      SCIMFilter filter,
      String comparator,
      Map<String, Object> values,
      String valuePrefix,
    ...
    if (filter.getFilterValue() == null) {
      ...
      if (value instanceof String) {
        // lower is used to satisfy the requirement that all quoted values are compared case
        // insensitive
        ...
        if (isDbCaseInsensitive()) {
          ...
          return "LOWER("
              + getAttributeName(filter, mapper)
              + ") "
              + comparator
              + " LOWER("
              + paramName
