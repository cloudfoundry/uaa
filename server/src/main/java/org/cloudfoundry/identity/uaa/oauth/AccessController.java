...
/**
 * Controller for retrieving the model for and displaying the confirmation page for access to a
 * protected resource.
 */
@Controller
...
public class AccessController {
  ...
  /**
   * @param useSsl Explicitly requests caller to point back to an authorization endpoint on "https",
   *     even if the incoming request is "http" (e.g. when downstream of the SSL termination behind
   *     a load balancer). Always use HTTPS if deployed on cloudfoundry null to use the incoming
   *     request to determine the URL scheme
   */
  public AccessController(
      final @Qualifier("jdbcClientDetailsService") MultitenantClientServices clientDetailsService,
      final @Value(
              "#{@applicationProperties['oauth.authorize.ssl']?:(T(java.lang.System).getenv('VCAP_APPLICATION')!=null ? true : null)}")
          Boolean useSsl,
  ...
  public String confirm(
    ...
    AuthorizationRequest clientAuthRequest =
    ...
    if (clientAuthRequest == null) {
      ...
      List<Approval> approvals =
          approvalStore.getApprovals(
              Origin.getUserId((Authentication) principal),
              clientId,
  ...
  private int codeIsPasswordOrOpenId(String code) {
    return code.startsWith(SCOPE_PREFIX + "password") || code.startsWith(SCOPE_PREFIX + "openid")
        ? 1
        : 0;
  }
  ...
  private String getRedirectUri(ClientDetails client, AuthorizationRequest clientAuth) {
    ...
    if (client.getRegisteredRedirectUri() != null
        && !client.getRegisteredRedirectUri().isEmpty()
        && result == null) {
