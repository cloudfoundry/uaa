...
public class OpenIdConnectEndpoints {
  ...
  public OpenIdConnectEndpoints(
      final @Value("${issuer.uri}") String issuer, final IdentityZoneManager identityZoneManager) {
    this.issuer = issuer;
  ...
  private String getTokenEndpoint() throws URISyntaxException {
    return UaaTokenUtils.constructTokenEndpointUrl(
        issuer, identityZoneManager.getCurrentIdentityZone());
  }
}
