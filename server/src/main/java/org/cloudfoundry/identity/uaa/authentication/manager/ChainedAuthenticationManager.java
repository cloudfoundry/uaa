...
public class ChainedAuthenticationManager implements AuthenticationManager {
    ...
    for (int i = 0; shallContinue && i < delegates.length; i++) {
      ...
      if (shallAuthenticate) {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "Attempting chained authentication of "
                  + output
                  + " with manager:"
                  + delegates[i].getAuthenticationManager()
                  + " required:"
                  + delegates[i].getRequired());
        }
        Authentication thisAuth = null;
        try {
          thisAuth =
              delegates[i].getAuthenticationManager().authenticate(auth != null ? auth : output);
        } catch (AuthenticationException x) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "Chained authentication exception:"
                    + x.getMessage()
                    + " at:"
          ...
          if (delegates[i].getStopIf() != null) {
            for (Class<? extends AuthenticationException> exceptionClass :
                delegates[i].getStopIf()) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "Chained Authentication status of "
                + output
                + " with manager:"
                + delegates[i]
                + "; Authenticated:"
  ...
  public static class AuthenticationManagerConfiguration {
    ...
    public AuthenticationManagerConfiguration(
        AuthenticationManager authenticationManager, String required) {
    ...
    public void setRequired(String required) {
      boolean valid = false;
      if (IF_PREVIOUS_FALSE.equals(required) || IF_PREVIOUS_TRUE.equals(required)) {
        valid = true;
      }

      if (!valid) {
        throw new IllegalArgumentException(
            required + " is not a valid value for property 'required'");
      }
