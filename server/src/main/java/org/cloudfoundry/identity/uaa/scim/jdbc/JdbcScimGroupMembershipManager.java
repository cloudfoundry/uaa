...
public class JdbcScimGroupMembershipManager implements ScimGroupMembershipManager {
  ...
  public static final String DYNAMIC_GET_GROUPS_BY_MEMBER_SQL_BASE =
      String.format(
          "select %s from %s g, %s gm where gm.group_id = g.id and gm.identity_zone_id = g.identity_zone_id and gm.identity_zone_id = ? and gm.member_id in (",
          "g." + JdbcScimGroupProvisioning.GROUP_FIELDS.replace(",", ",g."),
          JdbcScimGroupProvisioning.GROUP_TABLE,
          MEMBERSHIP_TABLE);
  private static final String ADD_MEMBER_SQL =
      String.format(
          "insert into %s ( %s ) values (?,?,?,?,?,?,?)",
          MEMBERSHIP_TABLE, MEMBERSHIP_FIELDS + ",identity_zone_id");

  private static final String GET_GROUPS_BY_MEMBER_SQL =
      String.format(
  ...
  private static final String GET_MEMBER_SQL =
      String.format(
          "select %s from %s where member_id=? and group_id=? and identity_zone_id=?",
          MEMBERSHIP_FIELDS, MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBER_WITH_ORIGIN_SQL =
      String.format(
          "delete from %s where member_id=? and origin = ? and identity_zone_id=?",
          MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBER_SQL =
      String.format(
          "delete from %s where member_id=? and group_id = ? and identity_zone_id=?",
          MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBERS_WITH_ORIGIN_GROUP_SQL =
      String.format("delete from %s where origin=? and identity_zone_id=?", MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBERS_IN_GROUP_SQL =
      String.format("delete from %s where group_id=? and identity_zone_id=?", MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBER_IN_GROUPS_SQL_USER =
      String.format(
          "delete from %s where member_id=? and member_type='USER' and identity_zone_id=?",
          MEMBERSHIP_TABLE);
  private static final String DELETE_MEMBER_IN_GROUPS_SQL_GROUP =
      String.format(
          "delete from %s where member_id=? and member_type='GROUP' and identity_zone_id=?",
          MEMBERSHIP_TABLE);
  private static final String GROUP_TABLE = "groups";
  private static final String GET_GROUPS_BY_EXTERNAL_MEMBER_SQL =
      String.format(
  ...
  private Set<ScimGroup> getDefaultUserGroups(String zoneId) {
    ...
    return zoneDefaultGroups.stream()
  ...
  @Override
  public ScimGroupMember addMember(
      final String groupId, final ScimGroupMember member, final String zoneId)
  ...
  public List<ScimGroupMember> getMembers(
      final String groupId, boolean includeEntities, String zoneId)
      throws ScimResourceNotFoundException {
    List<ScimGroupMember> result = jdbcTemplate.query(GET_MEMBERS_SQL, rowMapper, groupId, zoneId);

    if (includeEntities) {
      for (ScimGroupMember member : result) {
        if (member.getType().equals(ScimGroupMember.Type.USER)) {
          ScimUser user =
              userProvisioning.retrieve(member.getMemberId(), IdentityZoneHolder.get().getId());
          member.setEntity(user);
        } else if (member.getType().equals(ScimGroupMember.Type.GROUP)) {
          ScimGroup group =
              scimGroupProvisioning.retrieve(
                  member.getMemberId(), IdentityZoneHolder.get().getId());
  ...
  @Override
  public Set<ScimGroup> getGroupsWithMember(
      final String memberId, boolean transitive, String zoneId)
      throws ScimResourceNotFoundException {
  ...
  private void getGroupsWithMember(
    ...
    try {
      while (!memberList.isEmpty()) {
        int size =
            maxSqlParameters > 1
                ? Math.min(maxSqlParameters - 1, memberList.size())
                : memberList.size();
        StringBuilder builder = new StringBuilder(DYNAMIC_GET_GROUPS_BY_MEMBER_SQL_BASE);
        builder.append(
            memberList.subList(0, size).stream().map(s -> "?").collect(Collectors.joining(", ")));
        builder.append(");");
        Object[] parameterList =
            ArrayUtils.addAll(new Object[] {zoneId}, memberList.subList(0, size).toArray());
        groups.addAll(
            jdbcTemplate.query(builder.toString(), new ScimGroupRowMapper(), parameterList));
  ...
  @Override
  public Set<ScimGroup> getGroupsWithExternalMember(
      final String memberId, final String origin, String zoneId)
      throws ScimResourceNotFoundException {
    List<ScimGroup> results;
    ...
    try {
      results =
          jdbcTemplate.query(
              GET_GROUPS_BY_EXTERNAL_MEMBER_SQL,
              ps -> {
  ...
  @Override
  public ScimGroupMember getMemberById(String groupId, String memberId, String zoneId)
  ...
  @Override
  public List<ScimGroupMember> updateOrAddMembers(
      String groupId, List<ScimGroupMember> members, String zoneId)
      throws ScimResourceNotFoundException {
  ...
  @Override
  public ScimGroupMember removeMemberById(
      final String groupId, final String memberId, final String zoneId)
      throws ScimResourceNotFoundException, MemberNotFoundException {
    ...
    if (deleted != 1) {
      throw new IncorrectResultSizeDataAccessException(
          "unexpected number of members removed", 1, deleted);
    }
    return member;
  }

  @Override
  public List<ScimGroupMember> removeMembersByGroupId(final String groupId, final String zoneId)
      throws ScimResourceNotFoundException {
    ...
    if (deleted != members.size()) {
      throw new IncorrectResultSizeDataAccessException(
          "unexpected number of members removed", members.size(), deleted);
    }
  ...
  @Override
  public Set<ScimGroup> removeMembersByMemberId(final String memberId, final String zoneId)
      throws ScimResourceNotFoundException {
    ...
    int expectedDelete =
        isUser(memberId) ? groups.size() - getDefaultUserGroups(zoneId).size() : groups.size();
    if (deleted != expectedDelete) {
      throw new IncorrectResultSizeDataAccessException(
          "unexpected number of members removed", expectedDelete, deleted);
    }
  ...
  @Override
  public Set<ScimGroup> removeMembersByMemberId(
      final String memberId, final String origin, final String zoneId)
      throws ScimResourceNotFoundException {
    Set<ScimGroup> groups = getGroupsWithMember(memberId, false, zoneId);
    logger.debug("removing " + memberId + " from groups: " + groups);
    int deleted;
  ...
  private void validateRequest(String groupId, ScimGroupMember member, String zoneId) {
    if (!hasText(groupId) || !hasText(member.getMemberId()) || !hasText(member.getOrigin())) {
    ...
    ScimGroup group =
        scimGroupProvisioning.retrieve(
            groupId, IdentityZoneHolder.get().getId()); // this will throw a ScimException
    String memberZoneId;
    ...
    if (member.getType() == ScimGroupMember.Type.GROUP) {
      memberZoneId =
          scimGroupProvisioning
              .retrieve(member.getMemberId(), IdentityZoneHolder.get().getId())
              .getZoneId();
    } else {
      memberZoneId =
          userProvisioning
  ...
  @Override
  public void deleteMembersByOrigin(String origin, String zoneId)
      throws ScimResourceNotFoundException {
