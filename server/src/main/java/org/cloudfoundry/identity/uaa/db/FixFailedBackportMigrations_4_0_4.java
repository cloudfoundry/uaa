 ...
 *
 * <p>A back port of DB migration <code>4.0.1</code> to UAA 3.9.9 missed that fact that several
 * migrations where made in 3.10.0 This restores these migrations
 */
public class FixFailedBackportMigrations_4_0_4 implements JdbcMigration {

  private static final Logger logger =
      LoggerFactory.getLogger(FixFailedBackportMigrations_4_0_4.class);
  ...
  public void migrate(Connection connection) {
    ...
    for (Map.Entry<String, String> script : getScripts()) {
      ...
      if (count == 0) {
        ...
        logger.info(
            String.format(
                "[4.0.4] Adding script for version %s with path %s", script.getKey(), path));
