...
public class LegacyRedirectResolver
    extends org.cloudfoundry.identity.uaa.oauth.beans.org.springframework.security.oauth2.provider
        .endpoint.DefaultRedirectResolver {

  static final String MSG_TEMPLATE =
      "OAuth client %s is configured with a redirect_uri which performs implicit or "
  ...
  private final SpecCompliantRedirectMatcher specCompliantRedirectMatcher =
      new SpecCompliantRedirectMatcher();
  ...
  private static void redactQueryParams(UriComponentsBuilder builder) {
    ...
    Map<String, List<String>> redactedParams =
        originalParams.entrySet().stream()
            .map(
                e ->
                    new SimpleEntry<>(
                        e.getKey(), e.getValue().stream().map(v -> "REDACTED").collect(toList())))
  ...
  protected boolean redirectMatches(String requestedRedirect, String clientRedirect) {
    try {
      ...
      ClientRedirectUriPattern clientRedirectUri =
          new ClientRedirectUriPattern(normalizedClientRedirect);
      ...
      logger.error(
          String.format(
              "Could not validate whether requestedRedirect (%s) matches clientRedirectUri (%s)",
              requestedRedirect, clientRedirect),
          e);
      return false;
    }
  }
  ...
  public String resolveRedirect(String requestedRedirect, ClientDetails client)
    ...
    Set<String> registeredRedirectUris =
        ofNullable(client.getRegisteredRedirectUri()).orElse(emptySet());
    ...
    if (!invalidUrls.isEmpty()) {
      throw new RedirectMismatchException(
          "Client registration contains invalid redirect_uri: " + invalidUrls);
    }
    ...
    logConfiguredRedirectUrisWhichOnlyMatchFuzzily(
        client.getClientId(), registeredRedirectUris, requestedRedirect);

    return resolveRedirect;
  }

  private void logConfiguredRedirectUrisWhichOnlyMatchFuzzily(
      String clientId, Set<String> registeredRedirectUris, String requestedRedirect) {
        ...
        .filter(
            registeredRedirectUri ->
                requestedRedirect != null
                    ...
                    && !specCompliantRedirectMatcher.redirectMatches(
                        requestedRedirect, registeredRedirectUri))
        .forEach(
            registeredRedirectUri ->
                logger.warn(
                    String.format(
  ...
  private static class ClientRedirectUriPattern {
    ...
    private static final int URI_EXTRACTOR_AUTHORITY_GROUP =
        4; // "Authority" means "user:password@example.com"
    ...
    boolean isSafeRedirect(URI requestedRedirect) {
      ...
      boolean isSafe = true;
      for (int i = 0;
          i < configuredRedirectHost.length && !isWildcard(configuredRedirectHost[i]);
          i++) {
      ...
      if (hasWildcardPort) {
        if (requestedRedirect.getPort() > 0) {
          return matcher.match(
              ...
              requestedRedirect
                  .toString()
                  .replace(String.valueOf(requestedRedirect.getPort()), WILDCARD_PORT));
        } else {
          return matcher.match(
              redirectUri.replace(WILDCARD_PORT_PATTERN, StringUtils.EMPTY),
  ...
  private class SpecCompliantRedirectMatcher {
    ...
    private class CurrentVersionOfSpringResolverWithMethodExposedAndSubdomainsOff
