...
/**
 * An authentication manager that can be used to login to a remote UAA service with username and
 * password credentials, without the local server needing to know anything about the user accounts.
 * The request is handled by the UAA's RemoteAuhenticationEndpoint and success or failure is
 * determined by the response code.
 *
 * @author Dave Syer
 * @author Luke Taylor
 */
public class RestAuthenticationManager implements AuthenticationManager {
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    ...
    ResponseEntity<Map> response =
        restTemplate.exchange(
            remoteUrl,
            HttpMethod.POST,
            new HttpEntity<Object>(getParameters(username, password), headers),
            Map.class);
    ...
    if (response.getStatusCode() == HttpStatus.OK
      ...
      if (evaluateResponse(authentication, response)) {
        ...
        return new UsernamePasswordAuthenticationToken(
            username, nullPassword ? null : "", UaaAuthority.USER_AUTHORITIES);
      }
  ...
  protected Object getParameters(String username, String password) {
    MultiValueMap<String, Object> parameters =
        new LinkedMaskingMultiValueMap<String, Object>("password");
