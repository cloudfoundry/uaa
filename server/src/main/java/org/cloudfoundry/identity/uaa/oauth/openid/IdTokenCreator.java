...
public class IdTokenCreator {
  ...
  public IdTokenCreator(
      final TokenEndpointBuilder tokenEndpointBuilder,
  ...
  public IdToken create(
      String clientId, String userId, UserAuthenticationData userAuthenticationData)
    ...
    String givenName =
        getIfScopeContainsProfile(uaaUser.getGivenName(), userAuthenticationData.scopes);
    String familyName =
        getIfScopeContainsProfile(uaaUser.getFamilyName(), userAuthenticationData.scopes);
    String phoneNumber =
        getIfScopeContainsProfile(uaaUser.getPhoneNumber(), userAuthenticationData.scopes);

    String issuerUrl =
        tokenEndpointBuilder.getTokenEndpoint(identityZoneManager.getCurrentIdentityZone());
    ...
    ClientDetails clientDetails =
        multitenantClientServices.loadClientByClientId(clientId, identityZoneId);
    String clientTokenSalt =
        (String) clientDetails.getAdditionalInformation().get(ClientConstants.TOKEN_SALT);
    String revSig =
        getRevocableTokenSignature(
            uaaUser, clientTokenSalt, clientId, clientDetails.getClientSecret());
    return new IdToken(
  ...
  private Map<String, List<String>> buildUserAttributes(
      UserAuthenticationData userAuthenticationData, UaaUser user) {
    ...
    if (requestedAttributes && attributes == null) {
      logger.debug(
          String.format(
              "Requested id_token containing %s, but no saved attributes available for user with id:%s. Ensure storeCustomAttributes is enabled for origin:%s in zone:%s.",
              ClaimConstants.USER_ATTRIBUTES,
              user.getId(),
              user.getOrigin(),
  ...
  private Set<String> buildRoles(UserAuthenticationData userAuthenticationData, UaaUser user) {
    ...
    if (requestedRoles && roles == null) {
      logger.debug(
          String.format(
              "Requested id_token containing user roles, but no saved roles available for user with id:%s. Ensure storeCustomAttributes is enabled for origin:%s in zone:%s.",
              user.getId(), user.getOrigin(), identityZoneManager.getCurrentIdentityZoneId()));
    }
