...
public class ResetPasswordController {
  ...
  public String forgotPasswordPage(
    ...
    if (!IdentityZoneHolder.get()
        .getConfig()
        .getLinks()
        .getSelfService()
        .isSelfServiceLinksEnabled()) {
      return handleSelfServiceDisabled(
          model, response, "error_message_code", "self_service_disabled");
    }
  ...
  private void forgotPassword(String username, String clientId, String redirectUri) {
    ...
    try {
      ForgotPasswordInfo forgotPasswordInfo =
          resetPasswordService.forgotPassword(username, clientId, redirectUri);
  ...
  public String emailSentPage(@ModelAttribute("code") String code, HttpServletResponse response) {
  ...
  public String resetPasswordPage(
      Model model, HttpServletResponse response, @RequestParam("code") String code) {

    ExpiringCode expiringCode =
        checkIfUserExists(codeStore.retrieveCode(code, IdentityZoneHolder.get().getId()));
    if (expiringCode == null) {
      return handleUnprocessableEntity(model, response, "message_code", "bad_code");
    } else {
      PasswordChange passwordChange =
          JsonUtils.readValue(expiringCode.getData(), PasswordChange.class);
  ...
  private ExpiringCode checkIfUserExists(ExpiringCode code) {
    ...
    if (!hasText(code.getData())) {
      logger.debug(
          "reset_password ExpiringCode["
              + code.getCode()
              + "] data string is null or empty. Aborting.");
      return null;
    }
    Map<String, String> data =
        JsonUtils.readValue(code.getData(), new TypeReference<Map<String, String>>() {});
    ...
    try {
      ...
      logger.debug(
          "reset_password ExpiringCode[" + code.getCode() + "] user_id is invalid. Aborting.");
      return null;
    }
    return code;
  }
  ...
  private String handleUnprocessableEntity(
      Model model, HttpServletResponse response, String attributeKey, String attributeValue) {
    model.addAttribute(attributeKey, attributeValue);
    response.setStatus(HttpStatus.UNPROCESSABLE_ENTITY.value());
    return "forgot_password";
  }

  private String handleSelfServiceDisabled(
      Model model, HttpServletResponse response, String attributeKey, String attributeValue) {
    model.addAttribute(attributeKey, attributeValue);
