...
public class EncryptionKeyService {
  ...
  public EncryptionKeyService(
      ...
      final @Value("#{@config['encryption']['encryption_keys']}") List<EncryptionKey>
              encryptionKeys) {
    ...
    List<EncryptionKey> keysWithoutPassphrase =
    ...
    if (!keysWithoutPassphrase.isEmpty()) {
      throw new NoActiveEncryptionKeyProvided(
          String.format(
              "UAA cannot be started as encryption key passphrase for uaa.encryption.encryption_keys/[%s] is undefined",
              keysWithoutPassphrase.stream()
                  .map(s -> "label=" + s.getLabel())
                  .collect(Collectors.joining(", "))));
    }

    List<EncryptionKey> invalidLengthKeys =
        encryptionKeys.stream()
    ...
    if (!invalidLengthKeys.isEmpty()) {
      throw new NoActiveEncryptionKeyProvided(
          String.format(
              "The required length of the encryption passphrases for [%s] need to be at least 8 characters long.",
              invalidLengthKeys.stream()
                  .map(s -> "label=" + s.getLabel())
                  .collect(Collectors.joining(", "))));
    }
    ...
    if (!duplicateKeyLabels.isEmpty()) {
      throw new NoActiveEncryptionKeyProvided(
          String.format(
              "UAA cannot be started as multiple keys have the same label in uaa.encryption.encryption_keys/[%s]",
              duplicateKeyLabels.stream()
                  .map(s -> "label=" + s)
                  .collect(Collectors.joining(", "))));
    }
        ...
        encryptionKeys.stream()
            ...
            .orElseGet(
                () -> {
                  throw new NoActiveEncryptionKeyProvided(
                      String.format(
                          "UAA cannot be started as encryption key passphrase for uaa.encryption.encryption_keys/[label=%s] is undefined",
                          activeKeyLabel));
                });
  }
