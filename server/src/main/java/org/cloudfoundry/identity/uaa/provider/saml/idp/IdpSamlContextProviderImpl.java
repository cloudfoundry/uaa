...
/**
 * Use this class in conjuction with org.springframework.security.saml.SAMLProcessingFilter to
 * ensure that when SAMLProcessingFilter processes a SAML Authentication Request and builds a
...
public class IdpSamlContextProviderImpl extends SAMLContextProviderImpl {
  ...
  public SAMLMessageContext getLocalEntity(HttpServletRequest request, HttpServletResponse response)
    ...
    SAMLMessageContext context =
        super.getLocalEntity(new TestRequestWrapper(request), new TestResponseWrapper(response));
   ...
   * https(s)://server:port/application/saml/filterName/alias/aliasName/idp|sp. In case alias is
   * missing from the path defaults are used. Otherwise localEntityId and sp or idp localEntityRole
  ...
  protected void populateLocalEntityId(SAMLMessageContext context, String requestURI)
    ...
    entityId =
        (String)
            inTransport.getAttribute(
                org.springframework.security.saml.SAMLConstants.LOCAL_ENTITY_ID);
    if (entityId != null) {
