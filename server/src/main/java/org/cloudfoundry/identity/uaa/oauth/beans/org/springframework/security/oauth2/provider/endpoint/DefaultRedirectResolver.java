...
/**
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! This file was copied from Spring
 * Security Oauth2 v2.3.0. Our LegacyRedirectResolver is a subclass of this class. Newer versions of
 * the spring library changed this class's behavior significantly, and we need the old behavior for
 * our legacy mode (enabled by the Spring property uaa.oauth.redirect_uri.allow_unsafe_matching).
 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 *
 * <p>Default implementation for a redirect resolver.
 *
 * @author Ryan Heaton
 * @author Dave Syer
 */
public class DefaultRedirectResolver implements RedirectResolver {
  ...
  /**
   * Flag to indicate that requested URIs will match if they are a subdomain of the registered
   * value.
   *
  ...
  /**
   * Flag that enables/disables port matching between the requested redirect URI and the registered
   * redirect URI(s).
   *
   ...
   * would have the same host and root path if it is an HTTP URL. The port is also matched.
   *
  ...
  protected boolean redirectMatches(String requestedRedirect, String redirectUri) {
    try {
      ...
      if (reg.getProtocol().equals(req.getProtocol())
          ...
          && portsMatch) {
        return StringUtils.cleanPath(req.getPath())
            .startsWith(StringUtils.cleanPath(reg.getPath()));
      }
   ...
   *
   * @param redirectUris the set of the registered URIs to try and find a match. This cannot be null
   *     or empty.
