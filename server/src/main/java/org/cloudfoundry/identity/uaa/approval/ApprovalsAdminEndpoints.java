...
public class ApprovalsAdminEndpoints implements InitializingBean {
  ...
  public List<Approval> getApprovals(
      @RequestParam(required = false, defaultValue = "user_id pr") String ignored,
      @RequestParam(required = false, defaultValue = "1") int startIndex,
      @RequestParam(required = false, defaultValue = "100") int count) {
    String userId = getCurrentUserId();
    logger.debug("Fetching all approvals for user: " + userId);
    List<Approval> input =
        approvalStore.getApprovalsForUser(userId, IdentityZoneHolder.get().getId());
  ...
  public List<Approval> updateApprovals(@RequestBody Approval[] approvals) {
    ...
    for (Approval approval : approvals) {
      if (StringUtils.hasText(approval.getUserId()) && !isValidUser(approval.getUserId())) {
        logger.warn(
            String.format(
                "Error[2] %s attempting to update approvals for %s",
                currentUserId, approval.getUserId()));
  ...
  @ResponseBody
  public List<Approval> updateClientApprovals(
      @PathVariable String clientId, @RequestBody Approval[] approvals) {
    clientDetailsService.loadClientByClientId(clientId, IdentityZoneHolder.get().getId());
    String currentUserId = getCurrentUserId();
    logger.debug("Updating approvals for user: " + currentUserId);
    approvalStore.revokeApprovalsForClientAndUser(
        clientId, currentUserId, IdentityZoneHolder.get().getId());
    for (Approval approval : approvals) {
      if (StringUtils.hasText(approval.getUserId()) && !isValidUser(approval.getUserId())) {
        logger.warn(
            String.format(
                "Error[1] %s attemting to update approvals for %s.",
                currentUserId, approval.getUserId()));
        throw new UaaException(
            "unauthorized_operation",
            "Cannot update approvals for another user. Set user_id to null to update for existing user.",
  ...
  public ActionResult revokeApprovals(@RequestParam() String clientId) {
    ...
    return new ActionResult(
        "ok", "Approvals of user " + userId + " and client " + clientId + " revoked");
  }
  ...
  public View handleException(Exception t) {
    UaaException e =
        t instanceof UaaException
            ...
            : new UaaException(
                "Unexpected error",
                "Error accessing user's approvals",
    ...
    for (Class<?> key : statuses.keySet()) {
      if (key.isAssignableFrom(clazz)) {
        e =
            new UaaException(
                t.getMessage(), "Error accessing user's approvals", statuses.get(key).value());
        break;
      }
    }
    return new ConvertingExceptionView(
        new ResponseEntity<ExceptionReport>(
            new ExceptionReport(e, false), HttpStatus.valueOf(e.getHttpStatus())),
        messageConverters);
  }
