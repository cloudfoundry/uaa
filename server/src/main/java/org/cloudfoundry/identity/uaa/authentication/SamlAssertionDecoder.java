...
/**
 * Copy/paste from org.opensaml.saml2.binding.decoding.HTTPPostDecoder with two minor changes 1.
 * base64 decoding is doing base64url decoding 2. The unmarshalling of the object gets wrapped in a
 * SamlResponse object
 */
public class SamlAssertionDecoder extends BaseSAML2MessageDecoder {
  ...
  protected void doDecode(MessageContext messageContext) throws MessageDecodingException {
    ...
    Response response =
        SamlRedirectUtils.wrapAssertionIntoResponse(
            inboundMessage, inboundMessage.getIssuer().getValue());
   ...
   * @throws MessageDecodingException thrown if the message does not contain a base64 encoded SAML
   *     message
   */
  protected InputStream getBase64DecodedMessage(HTTPInTransport transport)
    ...
    if (DatatypeHelper.isEmpty(encodedMessage)) {
      log.error(
          "Request did not contain either a SAMLRequest or "
