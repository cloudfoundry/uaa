...
public class TokenRevocationEndpoint implements ApplicationEventPublisherAware {
  ...
  public TokenRevocationEndpoint(
      final @Qualifier("jdbcClientDetailsService") MultitenantJdbcClientDetailsService
  ...
  public ResponseEntity<Void> revokeTokensForUser(@PathVariable String userId) {
    ...
    eventPublisher.publishEvent(
        new TokenRevocationEvent(
            userId, null, zoneId, SecurityContextHolder.getContext().getAuthentication()));
    logger.debug("Tokens revoked for user: " + userId);
    return new ResponseEntity<>(OK);
  }

  @RequestMapping("/oauth/token/revoke/user/{userId}/client/{clientId}")
  public ResponseEntity<Void> revokeTokensForUserAndClient(
    ...
    eventPublisher.publishEvent(
        new TokenRevocationEvent(
            userId, clientId, zoneId, SecurityContextHolder.getContext().getAuthentication()));
  ...
  public ResponseEntity<Void> revokeTokensForClient(@PathVariable String clientId) {
    ...
    BaseClientDetails client =
        (BaseClientDetails) clientDetailsService.loadClientByClientId(clientId, zoneId);
    ...
    eventPublisher.publishEvent(
        new TokenRevocationEvent(
            null, clientId, zoneId, SecurityContextHolder.getContext().getAuthentication()));
  ...
  public ResponseEntity<List<RevocableToken>> listUserTokens(OAuth2Authentication authentication) {
    ...
    logger.debug(
        "Listing revocable tokens access token userId:" + userId + " clientId:" + clientId);
    List<RevocableToken> result =
        tokenProvisioning.getUserTokens(userId, clientId, IdentityZoneHolder.get().getId());
  ...
  public ResponseEntity<List<RevocableToken>> listUserTokens(
      @PathVariable String userId, OAuth2Authentication authentication) {
    if (OAuth2ExpressionUtils.hasAnyScope(
        authentication, new String[] {"tokens.list", "uaa.admin"})) {
      logger.debug("Listing revocable tokens for user:" + userId);
      List<RevocableToken> result =
          tokenProvisioning.getUserTokens(userId, IdentityZoneHolder.get().getId());
      removeTokenValues(result);
      return new ResponseEntity<>(result, OK);
    } else {
  ...
  public ResponseEntity<List<RevocableToken>> listClientTokens(
      @PathVariable String clientId, OAuth2Authentication authentication) {
    if (OAuth2ExpressionUtils.hasAnyScope(
        authentication, new String[] {"tokens.list", "uaa.admin"})) {
      logger.debug("Listing revocable tokens for client:" + clientId);
      List<RevocableToken> result =
          tokenProvisioning.getClientTokens(clientId, IdentityZoneHolder.get().getId());
      removeTokenValues(result);
      return new ResponseEntity<>(result, OK);
    } else {
      return listUserTokens(authentication);
  ...
  public ResponseEntity<OAuth2Exception> handleException(Exception e) throws Exception {
    ...
    InvalidTokenException e404 =
        new InvalidTokenException("Resource not found") {
          @Override
