...
public class IdpWebSsoProfileImpl extends WebSSOProfileImpl implements IdpWebSsoProfile {
  ...
  public AuthnRequest buildIdpInitiatedAuthnRequest(
  ...
  protected void buildResponse(
    ...
    AssertionConsumerService assertionConsumerService =
        getAssertionConsumerService(options, idpDescriptor, spDescriptor);
    ...
    Response samlResponse =
        createResponse(context, assertionConsumerService, assertion, authnRequest);
  ...
  private Response createResponse(
    ...
    buildCommonAttributes(
        context.getLocalEntityId(), response, assertionConsumerService, authnRequest);
  ...
  private void buildCommonAttributes(
      String localEntityId, Response response, Endpoint service, AuthnRequest authnRequest) {
  ...
  private Assertion buildAssertion(
    ...
    buildAssertionSubject(
        assertion,
        authnRequest,
        options.getAssertionTimeToLiveSeconds(),
  ...
  private void buildAssertionAuthnStatement(Assertion assertion) {
    @SuppressWarnings("unchecked")
    SAMLObjectBuilder<AuthnStatement> authnStatementBuilder =
    ...
    @SuppressWarnings("unchecked")
    SAMLObjectBuilder<AuthnContext> authnContextBuilder =
  ...
  private void buildAssertionConditions(
      Assertion assertion, int assertionTtlSeconds, String audienceURI) {
    @SuppressWarnings("unchecked")
    ...
    SAMLObjectBuilder<Audience> audienceBuilder =
        (SAMLObjectBuilder<Audience>) builderFactory.getBuilder(Audience.DEFAULT_ELEMENT_NAME);
  ...
  private void buildAssertionSubject(
      Assertion assertion,
      AuthnRequest authnRequest,
    ...
    if (null != authnRequest.getSubject()
        && null != authnRequest.getSubject().getNameID()
  ...
  protected void buildAttributeStatement(
      Assertion assertion, Authentication authentication, String providerEntityId) {
    @SuppressWarnings("unchecked")
    ...
    Attribute emailAttribute =
        buildStringAttribute("email", Collections.singletonList(principal.getEmail()));
    attributeStatement.getAttributes().add(emailAttribute);
    Attribute idAttribute =
        buildStringAttribute("id", Collections.singletonList(principal.getId()));
    attributeStatement.getAttributes().add(idAttribute);
    Attribute nameAttribute =
        buildStringAttribute("name", Collections.singletonList(principal.getName()));
    attributeStatement.getAttributes().add(nameAttribute);
    Attribute originAttribute =
        buildStringAttribute("origin", Collections.singletonList(principal.getOrigin()));
    attributeStatement.getAttributes().add(originAttribute);
    Attribute zoneAttribute =
        buildStringAttribute("zoneId", Collections.singletonList(principal.getZoneId()));
    attributeStatement.getAttributes().add(zoneAttribute);

    SamlServiceProviderDefinition config =
        samlServiceProviderProvisioning
            .retrieveByEntityId(providerEntityId, IdentityZoneHolder.get().getId())
            .getConfig();

    // static attributes
    for (Map.Entry<String, Object> staticAttribute :
        (ofNullable(config.getStaticCustomAttributes()).orElse(Collections.emptyMap()))
            .entrySet()) {
    ...
    if (attributeMappings.size() > 0) {
      ScimUser user =
          scimUserProvisioning.retrieve(principal.getId(), IdentityZoneHolder.get().getId());

      String givenName = user.getGivenName();
      if (StringUtils.hasText(givenName) && attributeMappings.containsKey("given_name")) {
        Attribute givenNameAttribute =
            buildStringAttribute(
                attributeMappings.get("given_name").toString(),
      ...
      if (StringUtils.hasText(familyName) && attributeMappings.containsKey("family_name")) {
        Attribute familyNameAttribute =
            buildStringAttribute(
                attributeMappings.get("family_name").toString(),
      ...
      if (StringUtils.hasText(phoneNumber) && attributeMappings.containsKey("phone_number")) {
        Attribute phoneNumberAttribute =
            buildStringAttribute(
                attributeMappings.get("phone_number").toString(),
      ...
      if (StringUtils.hasText(email) && attributeMappings.containsKey("email")) {
        Attribute customEmailAttribute =
            buildStringAttribute(
                attributeMappings.get("email").toString(), Collections.singletonList(email));
  ...
  public void setSamlServiceProviderProvisioning(
      JdbcSamlServiceProviderProvisioning samlServiceProviderProvisioning) {
