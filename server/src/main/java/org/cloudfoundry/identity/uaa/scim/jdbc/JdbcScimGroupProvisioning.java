...
public class JdbcScimGroupProvisioning extends AbstractQueryable<ScimGroup>
  ...
  protected static final String GROUP_FIELDS =
      "id,displayName,description,created,lastModified,version,identity_zone_id";
  protected static final String GROUP_TABLE = "groups";
  static final String ADD_GROUP_SQL =
      String.format("insert into %s ( %s ) values (?,?,?,?,?,?,?)", GROUP_TABLE, GROUP_FIELDS);
  ...
  private static final String UPDATE_GROUP_SQL =
      String.format(
          ...
          GROUP_TABLE);
  private static final String GET_GROUP_SQL =
      String.format(
          "select %s from %s where id=? and identity_zone_id=?", GROUP_FIELDS, GROUP_TABLE);
  private static final String GET_GROUP_BY_NAME_SQL =
      String.format(
          "select %s from %s where LOWER(displayName)=LOWER(?) and LOWER(identity_zone_id)=LOWER(?)",
          GROUP_FIELDS, GROUP_TABLE);
  private static final String QUERY_FOR_FILTER =
      String.format("select %s from %s", GROUP_FIELDS, GROUP_TABLE);
  private static final String DELETE_GROUP_SQL =
      String.format("delete from %s where id=? and identity_zone_id=?", GROUP_TABLE);
  private static final String DELETE_GROUP_BY_ZONE =
      String.format("delete from %s where identity_zone_id=?", GROUP_TABLE);
  private static final String DELETE_GROUP_MEMBERSHIP_BY_ZONE =
      String.format("delete from %s where identity_zone_id = ?", GROUP_MEMBERSHIP_TABLE);
  private static final String DELETE_EXTERNAL_GROUP_BY_ZONE =
      String.format("delete from %s where identity_zone_id = ?", EXTERNAL_GROUP_TABLE);
  private static final String DELETE_ZONE_ADMIN_MEMBERSHIP_BY_ZONE =
      String.format(
          "delete from %s where group_id in (select id from %s where identity_zone_id=? and displayName like ?)",
          GROUP_MEMBERSHIP_TABLE, GROUP_TABLE);
  private static final String DELETE_ZONE_ADMIN_GROUPS_BY_ZONE =
      String.format("delete from %s where identity_zone_id=? and displayName like ?", GROUP_TABLE);
  private static final String DELETE_GROUP_MEMBERSHIP_BY_PROVIDER =
      String.format(
          "delete from %s where identity_zone_id = ? and origin = ?", GROUP_MEMBERSHIP_TABLE);
  private static final String DELETE_EXTERNAL_GROUP_BY_PROVIDER =
      String.format(
          "delete from %s where identity_zone_id = ? and origin = ?", EXTERNAL_GROUP_TABLE);
  ...
  public JdbcScimGroupProvisioning(
      final JdbcTemplate jdbcTemplate, final JdbcPagingListFactory pagingListFactory) {
  ...
  public void setJdbcScimGroupExternalMembershipManager(
      final JdbcScimGroupExternalMembershipManager jdbcScimGroupExternalMembershipManager) {
    this.jdbcScimGroupExternalMembershipManager = jdbcScimGroupExternalMembershipManager;
  }

  public void setJdbcScimGroupMembershipManager(
  ...
  public ScimGroup getByName(String displayName, String zoneId) {
    ...
    List<ScimGroup> groups =
        jdbcTemplate.query(GET_GROUP_BY_NAME_SQL, rowMapper, displayName, zoneId);
    if (groups.size() == 1) {
      return groups.get(0);
    } else {
      throw new IncorrectResultSizeDataAccessException(
          "Invalid result size found for:" + displayName, 1, groups.size());
    }
  }
  ...
  public void onApplicationEvent(AbstractUaaEvent event) {
    if (event instanceof IdentityZoneModifiedEvent) {
      ...
      if (zevent.getEventType() == AuditEventType.IdentityZoneCreatedEvent) {
        ...
        getSystemScopes().forEach(scope -> createAndIgnoreDuplicate(scope, zoneId));
      }
    }
  ...
  @Override
  public ScimGroup create(final ScimGroup group, final String zoneId)
  ...
  @Override
  public ScimGroup delete(String id, int version, String zoneId)
  ...
  public int deleteByIdentityZone(String zoneId) {
    jdbcTemplate.update(
        DELETE_ZONE_ADMIN_MEMBERSHIP_BY_ZONE,
        IdentityZone.getUaaZoneId(),
        "zones." + zoneId + ".%");
    jdbcTemplate.update(
        DELETE_ZONE_ADMIN_GROUPS_BY_ZONE, IdentityZone.getUaaZoneId(), "zones." + zoneId + ".%");
