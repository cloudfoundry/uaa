  ...
  default void onApplicationEvent(EntityDeletedEvent<?> event) {
    if (event == null || event.getDeleted() == null) {
      ...
      getLogger()
          .debug(
              String.format(
                  "Received provider deletion event for zone_id:%s and origin:%s", zoneId, origin));
      ...
      getLogger()
          .debug(
              String.format(
                  "Received client deletion event for zone_id:%s and client:%s", zoneId, clientId));
      ...
      getLogger()
          .debug(
              String.format(
                  "Received UAA user deletion event for zone_id:%s and user:%s", zoneId, userId));
      ...
      getLogger()
          .debug(
              String.format(
                  "Received SCIM user deletion event for zone_id:%s and user:%s", zoneId, userId));
      deleteByUser(userId, zoneId);
