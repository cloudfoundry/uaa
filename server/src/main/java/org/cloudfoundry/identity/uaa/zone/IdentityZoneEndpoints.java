...
public class IdentityZoneEndpoints implements ApplicationEventPublisherAware {
  ...
  public IdentityZoneEndpoints(
      final IdentityZoneProvisioning zoneDao,
  ...
  public IdentityZone getIdentityZone(@PathVariable String id) {
    List<IdentityZone> result =
        filterForCurrentZone(Collections.singletonList(zoneDao.retrieveIgnoreActiveFlag(id)));
    if (result.size() == 0) {
  ...
  public ResponseEntity<IdentityZone> createIdentityZone(
      @RequestBody @Valid IdentityZone body, BindingResult result) {
    ...
    if (!IdentityZoneHolder.isUaa()) {
      throw new AccessDeniedException(
          "Zones can only be created by being authenticated in the default zone.");
    }
    ...
    try {
      ...
      throw new UnprocessableEntityException(
          "The identity zone details are invalid. " + errorMessage, ex);
    }
    ...
    try {
      logger.debug(
          "Zone - creating id[" + body.getId() + "] subdomain[" + body.getSubdomain() + "]");
      IdentityZone created = zoneDao.create(body);
      logger.debug(
          "Zone - created id[" + created.getId() + "] subdomain[" + created.getSubdomain() + "]");
      ...
      logger.debug(
          "Created default IDP in zone - created id["
              + created.getId()
              + "] subdomain["
              + created.getSubdomain()
              + "]");
  ...
  public void createUserGroups(IdentityZone zone) {
    ...
    if (userConfig != null) {
      List<String> defaultGroups =
          ofNullable(userConfig.getDefaultGroups()).orElse(Collections.emptyList());
      logger.debug(
          String.format(
              "About to create default groups count: %s for subdomain: %s",
              defaultGroups.size(), zone.getSubdomain()));
      for (String group : defaultGroups) {
        logger.debug(
            String.format(
                "Creating zone default group: %s for subdomain: %s", group, zone.getSubdomain()));
        groupProvisioning.createOrGet(new ScimGroup(null, group, zone.getId()), zone.getId());
      }
    }
  }
  ...
  public ResponseEntity<IdentityZone> updateIdentityZone(
    ...
    try {
      ...
      if (body.getId() != null && !body.getId().equals(id)) {
        throw new UnprocessableEntityException(
            "The identity zone id from the request body does not match id in the url");
      }
      ...
      logger.debug(
          "Zone - updated id[" + updated.getId() + "] subdomain[" + updated.getSubdomain() + "]");
  ...
  protected void restoreSecretProperties(IdentityZone existingZone, IdentityZone newZone) {
    if (newZone.getConfig() != null) {
      if (newZone.getConfig().getTokenPolicy() != null) {
        if (newZone.getConfig().getTokenPolicy().getKeys() == null
            || newZone.getConfig().getTokenPolicy().getKeys().isEmpty()) {
          newZone
              .getConfig()
              .getTokenPolicy()
              .setKeys(existingZone.getConfig().getTokenPolicy().getKeys());
        }
      }
  ...
  public ResponseEntity<ZoneAlreadyExistsException> handleZoneAlreadyExistsException(
      ZoneAlreadyExistsException e) {
  ...
  public ResponseEntity<ZoneDoesNotExistsException> handleZoneDoesNotExistsException(
      ZoneDoesNotExistsException e) {
  ...
  public ResponseEntity<UnprocessableEntityException> handleUnprocessableEntityException(
