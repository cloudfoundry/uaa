...
/**
 * Modern browser include the X-Requested-With header when making calls through the XMLHttpRequest
 * API which allows the server CORS filtering to mitigate against CSRF attacks performed by XHR
 * requests. However, in some situations XHR requests are useful. For example, when a single page
 * JavaScript apps that implements login using implicit grant wants to: 1) log the user out by
 * calling the /logout.do URI 2) get user information by calling the /userinfo URI.
 *
 * <p>To enable the scenarios described above, this filter allows CORS requests to include the
 * "X-Requested-With" header for a whitelist of URIs and origins and only for the HTTP GET method.
 *
 * <p>The implementation is based on guidance from: http://www.w3.org/TR/cors/
...
public class CorsFilter extends OncePerRequestFilter {
  ...
  public CorsFilter() {
    ...
    defaultConfiguration.setAllowedMethods(
        Arrays.asList(
            GET.toString(),
            OPTIONS.toString(),
            POST.toString(),
    ...
    xhrConfiguration.setAllowedHeaders(
        Arrays.asList(
            ACCEPT,
            ACCEPT_LANGUAGE,
            CONTENT_TYPE,
            ...
            AUTHORIZATION,
            X_REQUESTED_WITH));
    defaultConfiguration.setAllowedHeaders(
        Arrays.asList(ACCEPT, ACCEPT_LANGUAGE, CONTENT_TYPE, CONTENT_LANGUAGE, AUTHORIZATION));
      ...
      if (configuration.getAllowedUris() != null) {
        for (String allowedUri : configuration.getAllowedUris()) {
          try {
            ...
            logger.debug(
                String.format("URI '%s' is allowed for a %s CORS requests.", allowedUri, type));
          } catch (PatternSyntaxException patternSyntaxException) {
            logger.error(
                "Invalid regular expression pattern in cors."
                    + type
                    + ".allowed.uris: "
                    + allowedUri,
      ...
      if (configuration.getAllowedOrigins() != null) {
        for (String allowedOrigin : configuration.getAllowedOrigins()) {
          try {
            ...
            logger.debug(
                String.format(
                    "Origin '%s' is allowed for a %s CORS requests.", allowedOrigin, type));
          } catch (PatternSyntaxException patternSyntaxException) {
            logger.error(
                "Invalid regular expression pattern in cors."
                    + type
  ...
  protected boolean handleRequest(
    ...
    if (!isAllowedOrigin(origin, configuration)) {
      logger.debug(
          String.format(
              "Request with origin: %s was rejected because it didn't match allowed origins",
              origin));
    ...
    if (!isAllowedRequestUri(requestUri, configuration)) {
      logger.debug(
          String.format(
              "Request with URI: %s was rejected because it didn't match allowed URIs",
              requestUri));
  ...
  /**
   * Returns true if we believe this is an XHR request We look for the presence of the
   * X-Requested-With header or that the X-Requested-With header is listed as a value in the
  ...
  protected boolean isXhrRequest(final HttpServletRequest request) {
    ...
    return StringUtils.hasText(accessControlRequestHeaders)
        && containsHeader(accessControlRequestHeaders, X_REQUESTED_WITH);
  }
  ...
  protected void buildCorsPreFlightResponse(
      final HttpServletRequest request,
      final HttpServletResponse response,
    ...
    response.addHeader(
        ACCESS_CONTROL_ALLOW_METHODS, buildCommaDelimitedString(configuration.getAllowedMethods()));

    // we require Access-Control-Request-Headers header
    String accessControlRequestHeaders = request.getHeader(ACCESS_CONTROL_REQUEST_HEADERS);
    if (null == accessControlRequestHeaders) {
      response.sendError(
          BAD_REQUEST.value(), "Missing " + ACCESS_CONTROL_REQUEST_HEADERS + " header.");
      return;
    }
  ...
  protected boolean headersAllowed(
      final String accessControlRequestHeaders, CorsConfiguration configuration) {
    List<String> headers = splitCommaDelimitedString(accessControlRequestHeaders);
  ...
  public String getRequestInfo(HttpServletRequest request) {
    return String.format(
        "URI: %s; Scheme: %s; Host: %s; Port: %s; Origin: %s; Method: %s",
