...
public class NonSnarlMetadataManager extends MetadataManager
  ...
  public NonSnarlMetadataManager(SamlIdentityProviderConfigurator configurator)
  ...
  public List<ExtendedMetadataDelegate> getAvailableProviders() {
    ...
    for (SamlIdentityProviderDefinition definition :
        configurator.getIdentityProviderDefinitions()) {
      log.info(
          "Adding SAML IDP zone["
              + zone.getId()
              + "] alias["
  ...
  protected String getProviderIdpAlias(ExtendedMetadataDelegate provider)
      throws MetadataProviderException {
    List<String> stringSet = parseProvider(provider);
    for (String key : stringSet) {
      RoleDescriptor idpRoleDescriptor =
          provider.getRole(key, IDPSSODescriptor.DEFAULT_ELEMENT_NAME, SAMLConstants.SAML20P_NS);
  ...
  protected String getProviderSpAlias(ExtendedMetadataDelegate provider)
      throws MetadataProviderException {
    List<String> stringSet = parseProvider(provider);
    for (String key : stringSet) {
      RoleDescriptor spRoleDescriptor =
          provider.getRole(key, SPSSODescriptor.DEFAULT_ELEMENT_NAME, SAMLConstants.SAML20P_NS);
  ...
  protected String getHostedSpName(ExtendedMetadataDelegate provider)
      throws MetadataProviderException {
    List<String> stringSet = parseProvider(provider);
    for (String key : stringSet) {
      RoleDescriptor spRoleDescriptor =
  ...
  protected String getProviderAlias(ExtendedMetadataDelegate provider)
      throws MetadataProviderException {
    List<String> stringSet = parseProvider(provider);
    for (String key : stringSet) {
  ...
  /**
   * Method populates local storage of IDP and SP names and verifies any name conflicts which might
   * arise.
   *
  ...
  protected SignatureTrustEngine getTrustEngine(MetadataProvider provider) {
    ...
    if (verifyTrust) {
      ...
      return new AllowAllSignatureTrustEngine(
          Configuration.getGlobalSecurityConfiguration().getDefaultKeyInfoCredentialResolver());
    }
  }
  ...
  protected PKIXValidationInformationResolver getPKIXResolver(
      MetadataProvider provider, Set<String> trustedKeys, Set<String> trustedNames) {
    ...
    for (String key : trustedKeys) {
      log.debug(
          "Adding PKIX trust anchor {} for metadata verification of provider {}", key, provider);
      X509Certificate certificate = keyManager.getCertificate(key);
      if (certificate != null) {
        certificates.add(certificate);
      } else {
        log.warn(
            "Cannot construct PKIX trust anchor for key with alias {} for provider {}, key isn't included in the keystore",
            key,
            provider);
      }
    }
  ...
  /**
   * Parses entityID from the descriptor and adds it to the result set. Signatures on all found
   * entities are verified using the given policy and trust engine.
   *
    ...
    for (String idp : getIDPEntityNames()) {
      ...
      if (extendedMetadata.isLocal() && entityAlias.equals(extendedMetadata.getAlias())) {
        if (entityId != null && !entityId.equals(idp)) {
          throw new MetadataProviderException(
              "Alias " + entityAlias + " is used both for entity " + entityId + " and " + idp);
        } else {
          entityId = idp;
        }
      }
    ...
    for (String sp : getSPEntityNames()) {
      ...
      if (extendedMetadata.isLocal() && entityAlias.equals(extendedMetadata.getAlias())) {
        if (entityId != null && !entityId.equals(sp)) {
          throw new MetadataProviderException(
              "Alias " + entityAlias + " is used both for entity " + entityId + " and " + sp);
        } else {
          entityId = sp;
        }
      }
  ...
  public EntitiesDescriptor getEntitiesDescriptor(String name) {
    ...
    for (MetadataProvider provider : getProviders()) {
      ...
      try {
        ...
        log.warn(
            "Error retrieving metadata from provider of type {}, proceeding to next provider",
  ...
  public EntityDescriptor getEntityDescriptor(String entityID) {
    ...
    for (MetadataProvider provider : getProviders()) {
      log.debug(
          "Checking child metadata provider for entity descriptor with entity ID: {}", entityID);
      try {
