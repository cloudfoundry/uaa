...
public class YamlServletProfileInitializer implements ApplicationContextInitializer<ConfigurableWebApplicationContext> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(YamlServletProfileInitializer.class);
    ...
    public void initialize(ConfigurableWebApplicationContext applicationContext) {
        ...
        try {
            logger.info("Loading YAML environment properties from location: " + resources.toString());
            ...
            applyLog4jConfiguration(applicationContext.getEnvironment());

        } catch (Exception e) {
            logger.info("Error loading YAML environment properties from location: " + resources.toString(), e);
        }
    }
    ...
    private List<Resource> getResource(final ConfigurableWebApplicationContext applicationContext,
        ...
        for (String location : configFileLocations) {
            ...
            logger.info("Testing for YAML resources at: " + location);
    ...
    private void applyLog4jConfiguration(ConfigurableEnvironment environment) {
        ...
        if (environment.containsProperty("logging.config")) {
            ...
            if (location != null && location.trim().length() > 0) {
                ...
                if ((location.startsWith("-D") && environmentPropertySource != null && location.equals(environmentPropertySource.getProperty("LOGGING_CONFIG")))) {
                    logger.info("Ignoring Log Config Location: " + location + ". Location is suspect to be a Tomcat startup script environment variable");
                } else {
                    logger.info("Setting Log Config Location: " + location + " based on logging.config setting.");
                    log4jConfigLocation = environment.getProperty("logging.config");
                }
            }
        }

        logger.info("Loading log4j config from location: " + log4jConfigLocation);
        try {
            ...
            logger.info("Error loading log4j config from location: " + log4jConfigLocation, e);
        }
    }
    ...
    void applySpringProfiles(ConfigurableEnvironment environment) {
        ...
        logger.info("System property spring.profiles.active: [{}]", systemProfiles);

        environment.setDefaultProfiles();
        if (environment.containsProperty("spring_profiles")) {
            String profiles = environment.getProperty("spring_profiles");
            logger.info("Setting active profiles: " + profiles);
