...
/**
 * Simple controller that just returns "ok" in a request body for the purposes of monitoring health
 * of the application. It also registers a shutdown hook and returns "stopping" and a 503 when the
...
public class HealthzEndpoint {
  ...
  public HealthzEndpoint(
      @Value("${uaa.shutdown.sleep:10000}") final long sleepTime, final Runtime runtime) {
    Thread shutdownHook =
        new Thread(
            () -> {
              stopping = true;
              logger.warn(
                  "Shutdown hook received, future requests to this endpoint will return 503");
              try {
                if (sleepTime > 0) {
