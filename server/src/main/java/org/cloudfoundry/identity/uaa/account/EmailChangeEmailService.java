...
public class EmailChangeEmailService implements ChangeEmailService {
  ...
  public void beginEmailChange(
      String userId, String email, String newEmail, String clientId, String redirectUri) {
    ScimUser user =
    ...
    List<ScimUser> results =
        scimUserProvisioning.retrieveByUsernameAndOriginAndZone(
            newEmail, OriginKeys.UAA, identityZoneManager.getCurrentIdentityZoneId());
  ...
  private String generateExpiringCode(
      String userId, String newEmail, String clientId, String redirectUri) {
  ...
  public Map<String, String> completeVerification(String code) {
    ...
    if ((null == expiringCode)
        || ((null != expiringCode.getIntent()) && !EMAIL.name().equals(expiringCode.getIntent()))) {
    ...
    if (clientId != null) {
      ...
      try {
        ...
        String changeEmailRedirectUrl =
            (String) clientDetails.getAdditionalInformation().get(CHANGE_EMAIL_REDIRECT_URL);
        redirectLocation =
            findMatchingRedirectUri(redirectUris, redirectUri, changeEmailRedirectUrl);
  ...
  private String getSubjectText() {
    if (identityZoneManager.isCurrentZoneUaa()) {
      ...
      return StringUtils.hasText(companyName)
          ? companyName + " Email change verification"
  ...
  private String getEmailChangeEmailHtml(String email, String newEmail, String code) {
    String verifyUrl =
        UaaUrlUtils.getUaaUrl("/verify_email", identityZoneManager.getCurrentIdentityZone());
    ...
    if (identityZoneManager.isCurrentZoneUaa()) {
      ...
      ctx.setVariable(
          "servicePhrase",
          StringUtils.hasText(companyName) ? "a " + companyName + " account" : "an account");
    } else {
