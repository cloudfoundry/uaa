...
public class JdbcUserGoogleMfaCredentialsProvisioning
    implements SystemDeletable, UserMfaCredentialsProvisioning<UserGoogleMfaCredentials> {
  ...
  private static final String QUERY_USER_MFA_CONFIG_ALL_SQL =
      "SELECT * FROM user_google_mfa_credentials WHERE user_id=? AND mfa_provider_id=?";
  private static final String DELETE_USER_MFA_CONFIG_SQL =
      "DELETE FROM user_google_mfa_credentials WHERE user_id=?";
  private static final String DELETE_PROVIDER_MFA_CONFIG_SQL =
      "DELETE FROM user_google_mfa_credentials WHERE mfa_provider_id=?";
  private static final String DELETE_ZONE_MFA_CONFIG_SQL =
      "DELETE FROM user_google_mfa_credentials WHERE zone_id=?";
  private static Logger logger =
      LoggerFactory.getLogger(JdbcUserGoogleMfaCredentialsProvisioning.class);
  private JdbcTemplate jdbcTemplate;
  private UserMfaCredentialsMapper mapper;
  private EncryptionKeyService encryptionKeyService;

  public JdbcUserGoogleMfaCredentialsProvisioning(
      JdbcTemplate jdbcTemplate, EncryptionKeyService encryptionKeyService) {
  ...
  public UserGoogleMfaCredentials retrieve(String userId, String mfaProviderId) {
    try {
      return jdbcTemplate.queryForObject(
          QUERY_USER_MFA_CONFIG_ALL_SQL, mapper, userId, mfaProviderId);
  ...
  private static final class UserMfaCredentialsMapper
      implements RowMapper<UserGoogleMfaCredentials> {
    ...
    public UserGoogleMfaCredentials mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      if (StringUtils.isEmpty(encryptionKeyLabel)) {
        ...
        try {
          EncryptionKeyService.EncryptionKey encryptionKey =
              encryptionKeyService
                  .getKey(encryptionKeyLabel)
                  .orElseGet(
                      () -> {
                        RuntimeException cause =
                            new RuntimeException(
                                "Attempted to retrieve record with an unknown decryption key");
                        logger.error(
                            String.format(
                                "Couldn't decrypt with unknown key label : %s", encryptionKeyLabel),
                            cause);
              ...
              new UserGoogleMfaCredentials(
                  ...
                  new String(
                      encryptionKey.decrypt(
                          Base64Utils.decodeFromString(rs.getString("secret_key")))),
                  valueOf(
                      new String(
                          encryptionKey.decrypt(
                              Base64Utils.decodeFromString(
                                  rs.getString("encrypted_validation_code"))))),
                  fromSCString(
                      new String(
                          encryptionKey.decrypt(
                              Base64Utils.decodeFromString(rs.getString("scratch_codes"))))));
        } catch (EncryptionServiceException e) {
    ...
    private List<Integer> fromSCString(String csString) {
      return Arrays.stream(csString.split(","))
