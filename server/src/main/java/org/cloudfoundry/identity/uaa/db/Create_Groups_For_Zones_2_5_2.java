...
public class Create_Groups_For_Zones_2_5_2 implements SpringJdbcMigration {
  ...
  public void migrate(JdbcTemplate jdbcTemplate) {
    String groupCreateSQL =
        "INSERT INTO groups (id,displayName,created,lastModified,version,identity_zone_id) VALUES (?,?,?,?,?,?)";
    Map<String, Map<String, String>> zoneIdToGroupNameToGroupId = new HashMap<>();

    // duplicate all existing groups across zones
    List<String> zones =
        jdbcTemplate.queryForList("SELECT id FROM identity_zone WHERE id <> 'uaa'", String.class);
    List<String> groups =
        jdbcTemplate.queryForList(
            "SELECT displayName FROM groups WHERE identity_zone_id = 'uaa'", String.class);
    ...
    String userSQL =
        "SELECT gm.group_id, gm.member_id, g.displayName, u.identity_zone_id FROM group_membership gm, groups g, users u WHERE gm.member_type='USER' AND gm.member_id = u.id AND gm.group_id = g.id AND u.identity_zone_id <> 'uaa'";
    ...
    for (Map<String, Object> userRow : userMembers) {
      ...
      if (groupNameToGroupId == null) {
        ...
        if (count != 1) {
          logger.error(
              "Unable to delete membership for non existent zone(group:"
                  + oldGroupId
                  + ", member:"
                  + memberId
                  + ")");
        ...
        if (StringUtils.hasText(groupId)) {
          ...
          if (count != 1) {
            logger.error(
                "Unable to update group membership for migrated zone(old group:"
                    + oldGroupId
                    + ", member:"
                    + memberId
                    + ", new group:"
          ...
          logger.error(
              "Will not migrate (old group:"
                  + oldGroupId
                  + ", member:"
                  + memberId
                  + ", new group:"
                  + groupId
                  + "). Incorrectly mapped zones group? ("
                  + displayName
                  + ")");
        }
      }
