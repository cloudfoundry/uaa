...
/**
 * Event publisher for client registration changes with the resulting event type varying according
 * to the input and outcome. Can be used as an aspect intercepting calls to a component that changes
 * client details.
 */
public class ClientAdminEventPublisher implements ApplicationEventPublisherAware {
  ...
  public void create(ClientDetails client) {
    publish(
        new ClientCreateEvent(
            client, getPrincipal(), identityZoneManager.getCurrentIdentityZoneId()));
  }
    ...
    for (ClientDetailsModification client : clients) {
      if (ClientDetailsModification.ADD.equals(client.getAction())) {
        ...
        if (client.isApprovalsDeleted()) {
          publish(
              new SecretChangeEvent(
                  client, getPrincipal(), identityZoneManager.getCurrentIdentityZoneId()));
          publish(
              new ClientApprovalsDeletedEvent(
                  client, getPrincipal(), identityZoneManager.getCurrentIdentityZoneId()));
        }
  ...
  public void secretChange(String clientId) {
    publish(
        new SecretChangeEvent(
            getClient(clientId), getPrincipal(), identityZoneManager.getCurrentIdentityZoneId()));
  }
