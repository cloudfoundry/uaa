...
/**
 * This filter ensures that all requests are targeting a specific identity zone by hostname. If the
 * hostname doesn't match an identity zone, a 404 error is sent.
 */
public class IdentityZoneResolvingFilter extends OncePerRequestFilter implements InitializingBean {
  ...
  protected void doFilterInternal(
    ...
    if (subdomain != null) {
      try {
        ...
        String message =
            "Internal server error while fetching identity zone for subdomain" + subdomain;
    ...
    if (identityZone == null) {
      ...
      response.sendError(
          HttpServletResponse.SC_NOT_FOUND, "Cannot find identity zone for subdomain " + subdomain);
      return;
    }
    try {
  ...
  private String getSubdomain(String hostname) {
    ...
    logger.debug(
        "Unable to determine subdomain for host:"
            + hostname
            + "; root domains:"
  ...
  public void setAdditionalInternalHostnames(Set<String> hostnames) {
    if (hostnames != null) {
      hostnames.forEach(entry -> this.defaultZoneHostnames.add(entry.toLowerCase()));
    }
  }
  ...
  public void afterPropertiesSet() throws ServletException {
    ...
    logger.info(
        "Zone Resolving Root domains are: " + Arrays.toString(getDefaultZoneHostnames().toArray()));
  }
}
