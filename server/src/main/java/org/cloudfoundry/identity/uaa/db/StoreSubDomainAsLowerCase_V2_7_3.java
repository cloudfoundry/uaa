...
public class StoreSubDomainAsLowerCase_V2_7_3 implements SpringJdbcMigration {
  ...
  private RowMapper<IdentityZone> mapper =
      (rs, rowNum) -> {
  ...
  public synchronized void migrate(JdbcTemplate jdbcTemplate) {
      ...
      for (int i = 1; dupZones.size() > 1 && i < dupZones.size(); i++) {
        ...
        while (newsubdomain == null) {
          String potentialsubdomain =
              (dupZone.getSubdomain() + "-" + generator.generate()).toLowerCase();
        ...
        logger.debug(
            String.format(
                "Updating zone id:%s; old subdomain: %s; new subdomain: %s;",
    ...
    for (IdentityZone zone : identityZones) {
      ...
      if (StringUtils.hasText(subdomain) && !(subdomain.toLowerCase().equals(subdomain))) {
        logger.debug(
            String.format(
                "Lowercasing zone subdomain for id:%s; old subdomain: %s; new subdomain: %s;",
  ...
  private IdentityZone updateIdentityZone(IdentityZone identityZone, JdbcTemplate jdbcTemplate) {
    String ID_ZONE_UPDATE_FIELDS =
        "version,lastmodified,name,subdomain,description".replace(",", "=?,") + "=?";
    String UPDATE_IDENTITY_ZONE_SQL =
  ...
  private void addToMap(
      IdentityZone zone, Map<String, List<IdentityZone>> zones, Set<String> duplicates) {
