...
/**
 * Post processor which injects an additional filter at the head of each security filter chain.
 *
 * <p>If the requireHttps property is set, and a non HTTP request is received (as determined by the
 * absence of the <tt>httpsHeader</tt>) the filter will either redirect with a 301 or send an error
 * code to the client. Filter chains for which a redirect is required should be added to the
 * <tt>redirectToHttps</tt> list (typically those serving browser clients). Clients in this list
 * will also receive an HSTS response header, as defined in
 * http://tools.ietf.org/html/draft-ietf-websec-strict-transport-sec-14.
 *
 * <p>HTTP requests from any other clients will receive a JSON error message.
 *
 * <p>The filter also wraps calls to the <tt>getRemoteAddr</tt> to give a more accurate value for
 * the remote client IP, making use of the <tt>clientAddrHeader</tt> if available in the request.
 *
 * @author Luke Taylor
 */
@ManagedResource(
...
public class SecurityFilterChainPostProcessor implements BeanPostProcessor {
   ...
   * has been added to the head of the chain. Filters will be inserted in Map iteration order, at
   * the position given by the entry key (or the end of the chain if the key &gt; size).
   */
  ...
  final class HttpsEnforcementFilter extends UaaLoggingFilter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
      ...
      if (redirect) {
        ...
        response.sendError(
            HttpServletResponse.SC_BAD_REQUEST, "{\"error\": \"request must be over https\"}");
      }
    }
  }
