 ...
 * this application deployment. In case the application is configured to automatically generate
 * metadata, the generation occurs upon first invocation of this filter (first request made to the
 * server).
 *
 * <p>This class is based on org.springframework.security.saml.metadata.MetadataGeneratorFilter.
 */
public class IdpMetadataGeneratorFilter extends GenericFilterBean {

  /** Class logger. */
  protected static final Logger log = LoggerFactory.getLogger(IdpMetadataGeneratorFilter.class);
  ...
  /**
   * Flag indicates that in case generated base url is used (when value is not provided in the
   * MetadataGenerator) it should be normalized. Normalization includes lower-casing of scheme and
   * server name and removing standar ports of 80 for http and 443 for https schemes.
   */
  ...
  /**
   * Verifies whether generation is needed and if so the metadata document is created and stored in
   * metadata manager.
   *
      ...
      synchronized (IdpMetadataManager.class) {
        if (manager.getHostedIdpName() == null) {
          ...
          try {
            ...
            Collection<String> supportedNameID =
                Arrays.asList(NameIDType.EMAIL, NameIDType.PERSISTENT, NameIDType.UNSPECIFIED);
            ...
            log.info(
                "Created default metadata for system with entityID: " + descriptor.getEntityID());
            MetadataMemoryProvider memoryProvider = new MetadataMemoryProvider(descriptor);
            memoryProvider.initialize();
            MetadataProvider metadataProvider =
                new ExtendedMetadataDelegate(memoryProvider, extendedMetadata);
  ...
  /**
   * When true flag indicates that in case generated base url is used (when value is not provided in
   * the MetadataGenerator) it should be normalized. Normalization includes lower-casing of scheme
   * and server name and removing standar ports of 80 for http and 443 for https schemes.
   *
