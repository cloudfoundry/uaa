...
public class TokenKeyEndpoint {
  ...
  public TokenKeyEndpoint(final @Qualifier("keyInfoService") KeyInfoService keyInfoService) {
  ...
  @ResponseBody
  public ResponseEntity<VerificationKeyResponse> getKey(
      Principal principal,
  ...
  @ResponseBody
  public ResponseEntity<VerificationKeysListResponse> getKeys(
      Principal principal,
      @RequestHeader(value = "If-None-Match", required = false, defaultValue = "NaN") String eTag) {
    String lastModified = ((Long) IdentityZoneHolder.get().getLastModified().getTime()).toString();
    if (unmodifiedResource(eTag, lastModified)) {
  ...
  /**
   * Get the verification key for the token signatures. The principal has to be provided only if the
   * key is secret (shared not public).
   *
  ...
  /**
   * Get the verification key for the token signatures wrapped into keys array. Wrapping done for
   * compatibility with some clients expecting this even for single key, like mod_auth_openidc. The
   * principal has to be provided only if the key is secret (shared not public).
   *
   * @param principal the currently authenticated user if there is one
  ...
  public VerificationKeysListResponse getKeys(Principal principal) {
    ...
    List<VerificationKeyResponse> keyResponses =
        keys.values().stream()
