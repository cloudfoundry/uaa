...
public class JdbcClientMetadataProvisioning implements ClientMetadataProvisioning {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(JdbcClientMetadataProvisioning.class);

  private static final String CLIENT_METADATA_FIELDS =
      "client_id, identity_zone_id, show_on_home_page, app_launch_url, app_icon, additional_information, created_by";
  private static final String CLIENT_METADATA_QUERY =
      "select "
          + CLIENT_METADATA_FIELDS
          + " from oauth_client_details where client_id=? and identity_zone_id=?";
  private static final String CLIENT_METADATAS_QUERY =
      "select "
          + CLIENT_METADATA_FIELDS
          + " from oauth_client_details where identity_zone_id=? and (app_launch_url is not null or app_icon is not null)";
  private static final String CLIENT_METADATA_UPDATE_FIELDS =
      "show_on_home_page, app_launch_url, app_icon";
  private static final String CLIENT_METADATA_UPDATE =
      "update oauth_client_details set "
          + CLIENT_METADATA_UPDATE_FIELDS.replace(",", "=?,")
          + "=?"
        ...
        jdbcTemplate.update(
            ...
            ps -> {
              ...
              if (appIcon != null) {
                ...
                ps.setBinaryStream(
                    pos++, new ByteArrayInputStream(decodedAppIcon), decodedAppIcon.length);
              } else {
  ...
  protected void updateClientNameIfNotEmpty(ClientMetadata resource, String zoneId) {
    ...
    if (hasText(resource.getClientName())) {
      BaseClientDetails client =
          (BaseClientDetails)
              clientDetailsService.loadClientByClientId(resource.getClientId(), zoneId);
  ...
  private class ClientMetadataRowMapper implements RowMapper<ClientMetadata> {
    ...
    public ClientMetadata mapRow(ResultSet rs, int rowNum) throws SQLException {
      ...
      try {
        ...
        // it is safe to ignore this as client_metadata rows are always created from a
        // ClientMetadata instance whose launch url property is strongly typed to URL
      }
