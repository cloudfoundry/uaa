...
public class LoginSamlAuthenticationProvider extends SAMLAuthenticationProvider
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(LoginSamlAuthenticationProvider.class);
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    if (!supports(authentication.getClass())) {
      throw new IllegalArgumentException(
          "Only SAMLAuthenticationToken is supported, "
    ...
    logger.debug(
        String.format(
            "Initiating SAML authentication in zone '%s' domain '%s'",
    ...
    try {
      idp =
          identityProviderProvisioning.retrieveByOrigin(
              alias, identityZoneManager.getCurrentIdentityZoneId());
      ...
      throw new ProviderNotFoundException(
          "No SAML identity provider found in zone for alias:" + alias);
    }

    ExpiringUsernameAuthenticationToken result =
        getExpiringUsernameAuthenticationToken(authentication);
    UaaPrincipal samlPrincipal =
        new UaaPrincipal(
            NotANumber, result.getName(), result.getName(), alias, result.getName(), zone.getId());
    logger.debug(
        String.format(
            "Mapped SAML authentication to IDP with origin '%s' and username '%s'",
            idp.getOriginKey(), samlPrincipal.getName()));

    Collection<? extends GrantedAuthority> samlAuthorities =
        retrieveSamlAuthorities(samlConfig, (SAMLCredential) result.getCredentials());

    Collection<? extends GrantedAuthority> authorities = null;
    SamlIdentityProviderDefinition.ExternalGroupMappingMode groupMappingMode =
        idp.getConfig().getGroupMappingMode();
    ...
    MultiValueMap<String, String> userAttributes =
        retrieveUserAttributes(samlConfig, (SAMLCredential) result.getCredentials());

    if (samlConfig.getAuthnContext() != null) {
      if (Collections.disjoint(
          userAttributes.get(AUTHENTICATION_CONTEXT_CLASS_REFERENCE),
          samlConfig.getAuthnContext())) {
    ...
    UaaAuthentication resultUaaAuthentication =
        new LoginSamlAuthenticationToken(principal, result)
            .getUaaAuthentication(user.getAuthorities(), filteredExternalGroups, userAttributes);
    publish(
        new IdentityProviderAuthenticationSuccessEvent(
            user,
            resultUaaAuthentication,
            OriginKeys.SAML,
            identityZoneManager.getCurrentIdentityZoneId()));
    if (samlConfig.isStoreCustomAttributes()) {
      userDatabase.storeUserInfo(
          user.getId(),
          new UserInfo()
  ...
  protected ExpiringUsernameAuthenticationToken getExpiringUsernameAuthenticationToken(
  ...
  protected Set<String> filterSamlAuthorities(
      SamlIdentityProviderDefinition definition,
      Collection<? extends GrantedAuthority> samlAuthorities) {
    List<String> whiteList =
        of(definition.getExternalGroupsWhitelist()).orElse(Collections.EMPTY_LIST);
    Set<String> authorities =
        samlAuthorities.stream().map(s -> s.getAuthority()).collect(Collectors.toSet());
  ...
  protected Collection<? extends GrantedAuthority> mapAuthorities(
      String origin, Collection<? extends GrantedAuthority> authorities) {
    ...
    for (GrantedAuthority authority : authorities) {
      ...
      for (ScimGroupExternalMember internalGroup :
          externalMembershipManager.getExternalGroupMapsByExternalGroup(
              externalGroup, origin, identityZoneManager.getCurrentIdentityZoneId())) {
        String internalName = internalGroup.getDisplayName();
        logger.debug(
            String.format("Mapped external: '%s' to internal: '%s'", externalGroup, internalName));
  ...
  private Collection<? extends GrantedAuthority> retrieveSamlAuthorities(
      SamlIdentityProviderDefinition definition, SAMLCredential credential) {
    if (definition.getAttributeMappings().get(GROUP_ATTRIBUTE_NAME) != null) {
      ...
      credential.getAttributes().stream()
          .filter(
              attribute ->
                  groupAttributeNames.contains(attribute.getName())
                      || groupAttributeNames.contains(attribute.getFriendlyName()))
          ...
          .forEach(
              attribute -> {
                for (XMLObject group : attribute.getAttributeValues()) {
                  authorities.add(
                      new SamlUserAuthority(
                          getStringValue(attribute.getName(), definition, group)));
                }
              });
  ...
  private List<String> getGroupAttributeNames(SamlIdentityProviderDefinition definition) {
    ...
    if (definition.getAttributeMappings().get(GROUP_ATTRIBUTE_NAME) instanceof String) {
      ...
      attributeNames.addAll(
          (Collection) definition.getAttributeMappings().get(GROUP_ATTRIBUTE_NAME));
    }
    return attributeNames;
  }

  public MultiValueMap<String, String> retrieveUserAttributes(
      SamlIdentityProviderDefinition definition, SAMLCredential credential) {
    logger.debug(
        String.format(
            "Retrieving SAML user attributes [zone:%s, origin:%s]",
      ...
      for (Entry<String, Object> attributeMapping : definition.getAttributeMappings().entrySet()) {
        if (attributeMapping.getValue() instanceof String) {
          if (credential.getAttribute((String) attributeMapping.getValue()) != null) {
            ...
            for (XMLObject xmlObject :
                credential
                    .getAttribute((String) attributeMapping.getValue())
                    .getAttributeValues()) {
  ...
  protected String getStringValue(
      String key, SamlIdentityProviderDefinition definition, XMLObject xmlObject) {
    String value = null;
    ...
    if (value != null) {
      logger.debug(
          String.format(
              "Found SAML user attribute %s of value %s [zone:%s, origin:%s]",
              key, value, definition.getZoneId(), definition.getIdpEntityAlias()));
      return value;
  ...
  protected UaaUser createIfMissing(
    ...
    if (is_invitation_acceptance) {
      ...
      if (user.getUsername().equals(user.getEmail())
          && !user.getUsername().equals(samlPrincipal.getName())) {
    ...
    try {
      ...
      UaaUser uaaUser =
          userDatabase.retrieveUserByEmail(
              userWithSamlAttributes.getEmail(), samlPrincipal.getOrigin());
      if (uaaUser != null) {
        userModified = true;
        user = uaaUser.modifyUsername(samlPrincipal.getName());
      } else {
        if (!addNew) {
          throw new LoginSAMLException(
              "SAML user does not exist. "
                  + "You can correct this by creating a shadow user for the SAML user.",
              e);
        }
        publish(new NewUserAuthenticatedEvent(userWithSamlAttributes));
        try {
          user =
              userDatabase.retrieveUserByName(samlPrincipal.getName(), samlPrincipal.getOrigin());
  ...
  protected UaaUser getUser(UaaPrincipal principal, MultiValueMap<String, String> userAttributes) {
    ...
    return UaaUser.createWithDefaults(
        u ->
            u.withId(OriginKeys.NotANumber)
                ...
                .withVerified(
                    Boolean.valueOf(userAttributes.getFirst(EMAIL_VERIFIED_ATTRIBUTE_NAME)))
                .withOrigin(
                    principal.getOrigin() != null ? principal.getOrigin() : OriginKeys.LOGIN_SERVER)
                .withExternalId(name)
