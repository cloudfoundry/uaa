...
public class BackwardsCompatibleDelegatingPasswordEncoder implements PasswordEncoder {
  ...
  public BackwardsCompatibleDelegatingPasswordEncoder(
      final BCryptPasswordEncoder defaultPasswordEncoder) {
  ...
  public boolean matches(CharSequence rawPassword, String encodedPassword) {
    ...
    return defaultPasswordEncoder.matches(
        rawPassword, verifyPrefixAndExtractPassword(encodedPassword));
  }
  ...
  private String verifyPrefixAndExtractPassword(String encodedPassword) {
    ...
    if (!prefix.equals(OPTIONAL_BCRYPT_PREFIX)) {
      throw new IllegalArgumentException(
          String.format("Password encoding {%s} is not supported", prefix));
    }
