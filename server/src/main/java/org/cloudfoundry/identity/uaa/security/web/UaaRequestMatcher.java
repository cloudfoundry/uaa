...
/**
 * Custom request matcher which allows endpoints in the UAA to be matched as substrings and also
 * differentiation based on the content type (e.g. JSON vs HTML) specified in the Accept request
 * header, thus allowing different filter chains to be configured for browser and command-line
 * clients.
 *
 * <p>Currently just looks for a match of the configured MIME-type in the accept header when
 * deciding whether to match the request. There is no parsing of priorities in the header.
 */
public final class UaaRequestMatcher implements RequestMatcher, BeanNameAware {
  ...
  public UaaRequestMatcher(String path) {
    ...
    if (path.contains("*")) {
      throw new IllegalArgumentException(
          "UaaRequestMatcher is not intended for use with wildcards");
    }
    this.path = path;
  }
  ...
  /**
   * A media type that should be present in the accept header for a request to match. Optional (if
   * null then all values match).
   *
  ...
  /**
   * A map of request parameter name and values to match against. If all the specified parameters
   * are present and match the values given then the accept header will be ignored.
   *
