...
public class StatelessMfaAuthenticationFilter extends OncePerRequestFilter
  ...
  public StatelessMfaAuthenticationFilter(
      UserGoogleMfaCredentialsProvisioning provisioning,
    ...
    try {
      ...
      publishEvent(
          new MfaAuthenticationFailureEvent(
              user,
              getAuthentication(),
              provider != null ? provider.getType().toValue() : "null",
              IdentityZoneHolder.getCurrentZoneId()));
  ...
  protected MfaProvider checkMfaCode(HttpServletRequest request) {
    ...
    if (isMfaEnabled(zone)) {
      ...
      try {
        provider =
            mfaProvider.retrieveByName(
                zone.getConfig().getMfaConfig().getProviderName(), zone.getId());
      } catch (EmptyResultDataAccessException x) {
        throw new ProviderNotFoundException(
            "Unable to find MFA provider for zone:" + zone.getSubdomain());
      }
      Integer code = getMfaCode(request);
      UserGoogleMfaCredentials credentials =
          provisioning.getUserGoogleMfaCredentials(
              authentication.getPrincipal().getId(), provider.getId());
      if (credentials == null) {
        throw new UserMfaConfigDoesNotExistException(
            "User must register a multi-factor authentication token");
      }
