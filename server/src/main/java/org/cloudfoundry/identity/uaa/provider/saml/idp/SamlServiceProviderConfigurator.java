...
public class SamlServiceProviderConfigurator {
  ...
  private Set<String> supportedNameIDs =
      new HashSet<>(Arrays.asList(NameIDType.EMAIL, NameIDType.PERSISTENT, NameIDType.UNSPECIFIED));
  ...
  public List<SamlServiceProviderHolder> getSamlServiceProvidersForZone(IdentityZone zone) {
    ...
    for (SamlServiceProvider provider : providerProvisioning.retrieveActive(zone.getId())) {
      try {
        ...
        LOG.error(
            "Unable to configure SAML SP Metadata for ServiceProvider:" + provider.getEntityId(),
            e);
      }
    }
  ...
  synchronized void validateSamlServiceProvider(SamlServiceProvider provider, IdentityZone zone)
    ...
    if (!zone.getId().equals(provider.getIdentityZoneId())) {
      throw new IllegalArgumentException(
          "The SAML SP Identity Zone Id does not match the curent zone.");
    }
    ...
    // Initializing here is necessary to access the SPSSODescriptor, otherwise an exception is
    // thrown.
    added.initialize();
    SPSSODescriptor spSsoDescriptor =
        added.getEntityDescriptor(metadataEntityId).getSPSSODescriptor(SAMLConstants.SAML20P_NS);
    if (null != spSsoDescriptor
        && null != spSsoDescriptor.getNameIDFormats()
        && !spSsoDescriptor.getNameIDFormats().isEmpty()) {
      // The SP explicitly states the NameID formats it supports, we should check that we support at
      // least one.
      if (spSsoDescriptor.getNameIDFormats().stream()
          .noneMatch(format -> this.supportedNameIDs.contains(format.getFormat()))) {
        throw new MetadataProviderException(
    ...
    try {
      metadata =
          fixedHttpMetaDataProvider.fetchMetadata(
              def.getMetaDataLocation(), def.isSkipSslValidation());
