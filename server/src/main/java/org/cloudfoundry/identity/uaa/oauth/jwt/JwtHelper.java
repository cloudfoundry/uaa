...
public class JwtHelper {
   ...
   *
   * @param token the (non-null) encoded token (three Base-64 encoded strings separated by "."
   *     characters)
   */
  ...
  public static Jwt encode(CharSequence content, KeyInfo keyInfo) {
    JwtHeader header =
        JwtHeaderHelper.create(keyInfo.algorithm(), keyInfo.keyId(), keyInfo.keyURL());
 ...
 *
 * <p>Handles the JSON parsing and serialization.
 */
...
class JwtImpl implements Jwt {
   ...
   * @param content the base64-decoded "claims" segment (may be encrypted, depending on header
   *     information).
  ...
  private byte[] signingInput() {
    return concat(safeB64UrlEncode(header.bytes()), JwtHelper.PERIOD, safeB64UrlEncode(content));
  }
