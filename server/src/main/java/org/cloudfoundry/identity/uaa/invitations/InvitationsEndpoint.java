...
public class InvitationsEndpoint {
  ...
  public ResponseEntity<InvitationsResponse> inviteUsers(
    ...
    List<IdentityProvider> activeProviders =
        identityProviderProvisioning.retrieveActive(IdentityZoneHolder.get().getId());
    ...
    if (!hasText(subdomainHeader) && !hasText(zoneIdHeader)) {
      client =
          multitenantClientServices.loadClientByClientId(
              clientId, IdentityZoneHolder.get().getId());
    }
      ...
      try {
        if (email != null && validateEmail(email)) {
          ...
          if (providers.size() == 1) {
            ...
            String accountsUrl =
                UaaUrlUtils.getUaaUrl(
                    "/invitations/accept", !IdentityZoneHolder.isUaa(), IdentityZoneHolder.get());
              ...
              invitationsResponse
                  ...
                  .add(
                      InvitationsResponse.failure(
                          ...
                          String.format("Malformed url: %s", invitationLink)));
            }
          ...
          invitationsResponse
              ...
              .add(
                  InvitationsResponse.failure(
                      email, "email.invalid", email + " is invalid email."));
        }
  ...
  private ScimUser findOrCreateUser(String email, String origin) {
    ...
    List<ScimUser> results =
        scimUserProvisioning.retrieveByEmailAndZone(
            email, origin, IdentityZoneHolder.get().getId());
