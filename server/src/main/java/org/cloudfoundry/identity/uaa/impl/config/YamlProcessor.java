...
public class YamlProcessor {
   ...
   * YAML resource. In YAML documents are separated by <code>---</code> lines, and each document is
  ...
  /**
   * Flag indicating that a document that contains none of the keys in the {@link
   * #setDocumentMatchers(Map) document matchers} will nevertheless match.
   *
   ...
   *
   * <ul>
   *   <li><code>OVERRIDE</code> for replacing values from earlier in the list
   *   <li><code>OVERRIDE_AND_IGNORE</code> the same, but ignore IO errors loading individual
   *       resources
   *   <li><code>FIRST_FOUND</code> if you want to take the first resource in the list that exists
   *       and use just that.
   * </ul>
   *
   ...
   * #setDocumentMatchers(Map) matchers}. If a document matches it is passed into the callback,
   * along with its representation as Properties. Depending on the {@link
    ...
    for (Resource resource : resources) {
      try {
        ...
        if (e instanceof FileNotFoundException
            && e.getMessage().contains("${APPLICATION_CONFIG_FILE}")
    ...
    for (Entry<String, Object> entry : input.entrySet()) {
      ...
      if (value instanceof String) {
        ...
        for (Object object : collection) {
          assignProperties(
              properties, Collections.singletonMap("[" + (count++) + "]", object), key);
        }
      } else {
