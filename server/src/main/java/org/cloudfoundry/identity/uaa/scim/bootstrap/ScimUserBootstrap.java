...
public class ScimUserBootstrap
  ...
  private void deleteUsers(@NotNull List<String> deleteList) {
    ...
    List<ScimUser> list =
        scimUserProvisioning.query(
            "origin eq \"uaa\" and (" + filter.toString() + ")", IdentityZoneHolder.get().getId());
    for (ScimUser delete : list) {
      publish(
          new EntityDeletedEvent<>(
              delete,
              SystemAuthentication.SYSTEM_AUTHENTICATION,
      ...
      try {
        users =
            Collections.singletonList(
                scimUserProvisioning.retrieve(user.getId(), IdentityZoneHolder.get().getId()));
  ...
  private void updateUser(ScimUser existingUser, UaaUser updatedUser, boolean updateGroups) {
    ...
    if (updateGroups) {
      ...
      Set<ScimGroup> existingGroups =
          membershipManager.getGroupsWithMember(id, true, IdentityZoneHolder.get().getId());
    ...
    if (OriginKeys.UAA.equals(newScimUser.getOrigin())
        && hasText(updatedUser.getPassword())) { // password is not relevant for non UAA users
      scimUserProvisioning.changePassword(
          id, null, updatedUser.getPassword(), IdentityZoneHolder.get().getId());
    }
    if (updateGroups) {
  ...
  private void createNewUser(UaaUser user) {
    ...
    ScimUser newScimUser =
        scimUserProvisioning.createUser(
            convertToScimUser(user), user.getPassword(), IdentityZoneHolder.get().getId());
  ...
  public void onApplicationEvent(AuthEvent event) {
    ...
    if (event instanceof ExternalGroupAuthorizationEvent) {
      ...
      if (!OriginKeys.UAA.equals(origin)) {
        Set<ScimGroup> groupsWithMember =
            membershipManager.getGroupsWithExternalMember(
                exEvent.getUser().getId(), origin, IdentityZoneHolder.get().getId());
        Map<String, ScimGroup> groupsMap =
            groupsWithMember.stream()
                .collect(Collectors.toMap(ScimGroup::getDisplayName, Function.identity()));
        Collection<? extends GrantedAuthority> externalAuthorities =
            new LinkedHashSet<>(exEvent.getExternalAuthorities());
        for (GrantedAuthority authority : externalAuthorities) {
          if (groupsMap.containsKey(authority.getAuthority())) {
            ...
            addToGroup(
                exEvent.getUser().getId(), authority.getAuthority(), origin, exEvent.isAddGroups());
          }
        }
        for (ScimGroup group : groupsMap.values()) {
          membershipManager.removeMemberById(
              group.getId(), exEvent.getUser().getId(), group.getZoneId());
        }
      }
  ...
  private void addToGroup(String scimUserId, String gName, String origin, boolean addGroup) {
    ...
    List<ScimGroup> g =
        scimGroupProvisioning.query(
            String.format("displayName eq \"%s\"", gName), IdentityZoneHolder.get().getId());
    ScimGroup group;
  ...
  private void removeFromGroup(String scimUserId, String gName) {
    ...
    try {
      membershipManager.removeMemberById(
          group.getId(), scimUserId, IdentityZoneHolder.get().getId());
  ...
  private ScimUser convertToScimUser(UaaUser user) {
    ScimUser scim =
        new ScimUser(user.getId(), user.getUsername(), user.getGivenName(), user.getFamilyName());
