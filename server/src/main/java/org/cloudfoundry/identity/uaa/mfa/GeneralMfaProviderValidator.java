...
public class GeneralMfaProviderValidator implements MfaProviderValidator {
  ...
  public void validate(MfaProvider mfaProvider) {
    ...
    if (mfaProvider.getType() == null) {
      throw new InvalidMfaProviderException(
          "Provider type is required. Must be one of "
              + MfaProvider.MfaProviderType.getStringValues());
    }
    if (mfaProvider.getConfig() == null) {
      throw new InvalidMfaProviderException("Provider config is required");
    }
    if (mfaProvider.getConfig().getIssuer() != null
        && mfaProvider.getConfig().getIssuer().contains(":")) {
      throw new InvalidMfaProviderException(
          "Provider config contains an invalid issuer. Issuer must not contain a colon");
    }
