...
/**
 * Authorization endpoint that returns id_token's if requested. This is a copy of
 * AuthorizationEndpoint.java in Spring Security Oauth2. As that code does not allow for redirect
 * responses to be customized, as desired by
...
public class UaaAuthorizationEndpoint extends AbstractEndpoint implements AuthenticationEntryPoint {
  ...
  static final String ORIGINAL_AUTHORIZATION_REQUEST =
  ...
  private static final List<String> supported_response_types =
  ...
  UaaAuthorizationEndpoint(
      ...
      final @Qualifier("authorizationCodeServices") AuthorizationCodeServices
              authorizationCodeServices,
      final @Qualifier("hybridTokenGranterForAuthCodeGrant") HybridTokenGranterForAuthorizationCode
              hybridTokenGranterForAuthCode,
      final @Qualifier("openIdSessionStateCalculator") OpenIdSessionStateCalculator
  ...
  public ModelAndView authorize(
    ...
    // Pull out the authorization request first, using the OAuth2RequestFactory. All further logic
    // should
    // query off of the authorization request instead of referring back to the parameters map. The
    // contents of the
    // parameters map will be stored without change in the AuthorizationRequest object once it is
    // created.
    ...
    try {
      ...
      boolean isAuthenticated =
          (principal instanceof Authentication) && ((Authentication) principal).isAuthenticated();
      ...
      if (!(responseTypes.size() > 0)) {
        return new ModelAndView(
            new RedirectView(
                addQueryParameter(
                    addQueryParameter(resolvedRedirect, "error", "invalid_request"),
                    "error_description",
                    "Missing response_type in authorization request")));
      }

      authorizationRequest.setRedirectUri(resolvedRedirect);
      // We intentionally only validate the parameters requested by the client (ignoring any data
      // that may have
      ...
      if (authorizationRequest.isApproved()) {
        if (responseTypes.contains("token") || responseTypes.contains("id_token")) {
          return getImplicitGrantOrHybridResponse(
              authorizationRequest, (Authentication) principal, grantType);
        }
      ...
      if ("none".equals(authorizationRequest.getRequestParameters().get("prompt"))) {
        ...
        // so any auth request parameters passed to approveOrDeny will be ignored and retrieved from
        // the session.
      ...
      if ("none".equals(authorizationRequest.getRequestParameters().get("prompt"))) {
        return new ModelAndView(
            new RedirectView(
                addFragmentComponent(resolvedRedirect, "error=internal_server_error")));
      }
  ...
  // This method handles /oauth/authorize calls when user is not logged in and the prompt=none param
  // is used
  @Override
  public void commence(
    ...
    String[] responseTypes =
        ofNullable(request.getParameter(OAuth2Utils.RESPONSE_TYPE))
            .map(rt -> rt.split(" "))
            .orElse(new String[0]);
    ...
    String errorCode =
        authException instanceof InteractionRequiredException
  ...
  private ModelAndView switchIdp(
      Map<String, Object> model,
      ClientDetails client,
      String clientId,
      HttpServletRequest request) {
  ...
  Map<String, Object> unmodifiableMap(AuthorizationRequest authorizationRequest) {
    ...
    if (authorizationRequest.getResponseTypes() != null) {
      authorizationRequestMap.put(
          OAuth2Utils.RESPONSE_TYPE, Set.copyOf(authorizationRequest.getResponseTypes()));
    }
    if (authorizationRequest.getScope() != null) {
      authorizationRequestMap.put(OAuth2Utils.SCOPE, Set.copyOf(authorizationRequest.getScope()));
    }

    authorizationRequestMap.put("approved", authorizationRequest.isApproved());

    if (authorizationRequest.getResourceIds() != null) {
      authorizationRequestMap.put("resourceIds", Set.copyOf(authorizationRequest.getResourceIds()));
    }
    if (authorizationRequest.getAuthorities() != null) {
      authorizationRequestMap.put(
          "authorities", Set.<GrantedAuthority>copyOf(authorizationRequest.getAuthorities()));
    }
  ...
  public View approveOrDeny(
    ...
    AuthorizationRequest authorizationRequest =
        (AuthorizationRequest) model.get(AUTHORIZATION_REQUEST);
    ...
    Map<String, Object> originalAuthorizationRequest =
        (Map<String, Object>) model.get(ORIGINAL_AUTHORIZATION_REQUEST);
    if (isAuthorizationRequestModified(authorizationRequest, originalAuthorizationRequest)) {
      logger.warn("The requested scopes are invalid");
      throw new InvalidRequestException(
          "Changes were detected from the original authorization request.");
    }
  ...
  private boolean isAuthorizationRequestModified(
    ...
    if (!ObjectUtils.nullSafeEquals(
        authorizationRequest.getState(), originalAuthorizationRequest.get(OAuth2Utils.STATE))) {
      return true;
    }
    if (!ObjectUtils.nullSafeEquals(
    ...
    if (!ObjectUtils.nullSafeEquals(
        authorizationRequest.isApproved(), originalAuthorizationRequest.get("approved"))) {
      return true;
    }
    if (!ObjectUtils.nullSafeEquals(
        authorizationRequest.getResourceIds(), originalAuthorizationRequest.get("resourceIds"))) {
      return true;
    }
    if (!ObjectUtils.nullSafeEquals(
        authorizationRequest.getAuthorities(), originalAuthorizationRequest.get("authorities"))) {
      return true;
    }

    return !ObjectUtils.nullSafeEquals(
        authorizationRequest.getScope(), originalAuthorizationRequest.get(OAuth2Utils.SCOPE));
  }
  ...
  private ModelAndView getUserApprovalPageResponse(
      Map<String, Object> model,
      AuthorizationRequest authorizationRequest,
  ...
  private ModelAndView getImplicitGrantOrHybridResponse(
      AuthorizationRequest authorizationRequest, Authentication authentication, String grantType) {
    OAuth2AccessToken accessToken;
    try {
      TokenRequest tokenRequest =
          getOAuth2RequestFactory().createTokenRequest(authorizationRequest, GRANT_TYPE_IMPLICIT);
      Map<String, String> requestParameters =
          new HashMap<>(authorizationRequest.getRequestParameters());
      ...
      accessToken =
          getAccessTokenForImplicitGrantOrHybrid(tokenRequest, storedOAuth2Request, grantType);
      if (accessToken == null) {
  ...
  private OAuth2AccessToken getAccessTokenForImplicitGrantOrHybrid(
      TokenRequest tokenRequest, OAuth2Request storedOAuth2Request, String grantType)
      throws OAuth2Exception {
    // These 1 method calls have to be atomic, otherwise the ImplicitGrantService can have a race
    // condition where
    // one thread removes the token request before another has a chance to redeem it.
    synchronized (this.implicitLock) {
      switch (grantType) {
        case GRANT_TYPE_IMPLICIT:
          return getTokenGranter()
              .grant(grantType, new ImplicitTokenRequest(tokenRequest, storedOAuth2Request));
        case GRANT_TYPE_AUTHORIZATION_CODE:
          return hybridTokenGranterForAuthCode.grant(
              grantType, new ImplicitTokenRequest(tokenRequest, storedOAuth2Request));
        default:
  ...
  private View getAuthorizationCodeResponse(
      AuthorizationRequest authorizationRequest, Authentication authUser) {
    try {
      return new RedirectView(
  ...
  public String buildRedirectURI(
    ...
    if (accessToken instanceof CompositeToken
      ...
      url.append("&")
          .append(CompositeToken.ID_TOKEN)
          .append("=")
    ...
    if ("none".equals(authorizationRequest.getRequestParameters().get("prompt"))) {
      ...
      String sessionState =
          openIdSessionStateCalculator.calculate(
              ((UaaPrincipal) authUser.getPrincipal()).getId(),
              authorizationRequest.getClientId(),
              httpHost.toURI());
  ...
  private String generateCode(
      AuthorizationRequest authorizationRequest, Authentication authentication)
  ...
  private String getUnsuccessfulRedirect(
      AuthorizationRequest authorizationRequest, OAuth2Exception failure, boolean fragment) {
    ...
    if (failure.getAdditionalInformation() != null) {
      for (Map.Entry<String, String> additionalInfo :
        ...
        values
            .append("&")
            .append(encode(additionalInfo.getKey()))
            .append("=")
            .append(encode(additionalInfo.getValue()));
      }
    }
  ...
  public ModelAndView handleOAuth2Exception(OAuth2Exception e, ServletWebRequest webRequest)
      throws Exception {
  ...
  public ModelAndView handleHttpSessionRequiredException(
      HttpSessionRequiredException e, ServletWebRequest webRequest) throws Exception {
    logger.info("Handling Session required error: " + e.getMessage());
    return handleException(
        new AccessDeniedException("Could not obtain authorization request from session", e),
        webRequest);
  }
  ...
  private ModelAndView handleException(Exception e, ServletWebRequest webRequest) throws Exception {
    ...
    try {
      ...
      String requestedRedirectParam =
          authorizationRequest.getRequestParameters().get(OAuth2Utils.REDIRECT_URI);
      String requestedRedirect =
          redirectResolver.resolveRedirect(
              requestedRedirectParam, loadClientByClientId(authorizationRequest.getClientId()));
      ...
      // an error. OAuth2Exception can be handled this way. Other exceptions will generate a
      // standard 500
      // response.
  ...
  private AuthorizationRequest getAuthorizationRequestForError(ServletWebRequest webRequest) {
    ...
    AuthorizationRequest authorizationRequest =
        (AuthorizationRequest)
            sessionAttributeStore.retrieveAttribute(webRequest, AUTHORIZATION_REQUEST);
    if (authorizationRequest != null) {
