...
public class CurrentUserCookieRequestFilter extends OncePerRequestFilter {
  ...
  private String errorMessage(UaaPrincipal principal) {
    return String.format(
        "There was a problem while creating the Current-User cookie for user id %s",
        principal.getId());
  }

  private void handleError(HttpServletResponse response, UaaPrincipal principal)
      throws IOException {
    int status = HttpStatus.INTERNAL_SERVER_ERROR.value();
    UaaException error =
        new UaaException(CURRENT_USER_COOKIE_ERROR, errorMessage(principal), status);
