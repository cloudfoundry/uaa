...
public class ChangeEmailEndpoints implements ApplicationEventPublisherAware {
  ...
  public ResponseEntity<String> generateEmailVerificationCode(
      @RequestBody EmailChange emailChange) {
    ...
    try {
      final String code =
          expiringCodeStore
              .generateCode(
                  JsonUtils.writeValueAsString(emailChange),
  ...
  public ResponseEntity<EmailChangeResponse> changeEmail(@RequestBody String code) {
    ExpiringCode expiringCode =
        expiringCodeStore.retrieveCode(code, identityZoneManager.getCurrentIdentityZoneId());
    if ((null != expiringCode)
      ...
      if (clientId != null && !clientId.equals("")) {
        ClientDetails clientDetails =
            clientDetailsService.retrieve(clientId, identityZoneManager.getCurrentIdentityZoneId());
        redirectLocation =
            (String) clientDetails.getAdditionalInformation().get(CHANGE_EMAIL_REDIRECT_URL);
      }

      publisher.publishEvent(
          UserModifiedEvent.emailChanged(user, identityZoneManager.getCurrentIdentityZoneId()));
