...
public class JdbcUaaUserDatabase implements UaaUserDatabase {
  ...
  public static final String DEFAULT_UPDATE_USER_LAST_LOGON =
  ...
  private static final String USER_FIELDS =
      "id,username,password,email,givenName,familyName,created,lastModified,authorities,origin,external_id,verified,identity_zone_id,salt,passwd_lastmodified,phoneNumber,legacy_verification_behavior,passwd_change_required,last_logon_success_time,previous_logon_success_time ";

  private static final String PRE_DEFAULT_USER_BY_USERNAME_QUERY =
      "select "
          + USER_FIELDS
          + "from users where %s = ? and active=? and origin=? and identity_zone_id=?";
  static final String DEFAULT_CASE_SENSITIVE_USER_BY_USERNAME_QUERY =
      String.format(PRE_DEFAULT_USER_BY_USERNAME_QUERY, "lower(username)");
  static final String DEFAULT_CASE_INSENSITIVE_USER_BY_USERNAME_QUERY =
      String.format(PRE_DEFAULT_USER_BY_USERNAME_QUERY, "username");

  private static final String PRE_DEFAULT_USER_BY_EMAIL_AND_ORIGIN_QUERY =
      "select "
          + USER_FIELDS
          + "from users where %s=? and active=? and origin=? and identity_zone_id=?";
  static final String DEFAULT_CASE_SENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY =
      String.format(PRE_DEFAULT_USER_BY_EMAIL_AND_ORIGIN_QUERY, "lower(email)");
  static final String DEFAULT_CASE_INSENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY =
      String.format(PRE_DEFAULT_USER_BY_EMAIL_AND_ORIGIN_QUERY, "email");
  private static final String DEFAULT_USER_BY_ID_QUERY =
      "select " + USER_FIELDS + "from users where id = ? and active=? and identity_zone_id=?";
  ...
  public UaaUser retrieveUserByEmail(String email, String origin) throws UsernameNotFoundException {
    String sql =
        caseInsensitive
            ? DEFAULT_CASE_INSENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY
            : DEFAULT_CASE_SENSITIVE_USER_BY_EMAIL_AND_ORIGIN_QUERY;
    List<UaaUser> results =
        jdbcTemplate.query(
            sql,
            mapper,
    ...
    if (results.size() == 0) {
      ...
      throw new IncorrectResultSizeDataAccessException(
          String.format("Multiple users match email=%s origin=%s", email, origin),
          1,
          results.size());
    }
  }
  ...
  public UserInfo getUserInfo(String id) {
    try {
      return jdbcTemplate.queryForObject(
          "select user_id, info from user_info where user_id = ?", userInfoMapper, id);
    } catch (EmptyResultDataAccessException e) {
  ...
  public void updateLastLogonTime(String userId) {
    jdbcTemplate.update(
        DEFAULT_UPDATE_USER_LAST_LOGON,
  ...
  private final class UaaUserRowMapper implements RowMapper<UaaUser> {
    ...
    protected void getAuthorities(Set<String> authorities, final List<String> memberIdList) {
      ...
      while (!memberList.isEmpty()) {
        StringBuffer dynamicAuthoritiesQuery =
            new StringBuffer(
        ...
        Object[] parameterList =
            ArrayUtils.addAll(
                new Object[] {identityZoneManager.getCurrentIdentityZoneId()},
                memberList.subList(0, size).toArray());

        results.addAll(
            jdbcTemplate.queryForList(dynamicAuthoritiesQuery.toString(), parameterList));
