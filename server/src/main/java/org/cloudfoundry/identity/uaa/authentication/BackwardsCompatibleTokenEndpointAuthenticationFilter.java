...
/**
 * Provides an implementation that sets the UserAuthentication prior to createAuthorizatioRequest is
 * called. Backwards compatible with Spring Security Oauth2 v1 This is a copy of the
 * TokenEndpointAuthenticationFilter from Spring Security Oauth2 v2, but made to work with UAA
 */
public class BackwardsCompatibleTokenEndpointAuthenticationFilter implements Filter {

  private static final Logger logger =
      LoggerFactory.getLogger(BackwardsCompatibleTokenEndpointAuthenticationFilter.class);
    ...
    try {
      ...
      if (userAuthentication != null) {
        ...
        AuthorizationRequest authorizationRequest =
            oAuth2RequestFactory.createAuthorizationRequest(map);
        ...
        OAuth2Request storedOAuth2Request =
            oAuth2RequestFactory.createOAuth2Request(authorizationRequest);
      ...
      InsufficientAuthenticationException ex =
          new InsufficientAuthenticationException(message, failed);
  ...
  /**
   * If the incoming request contains user credentials in headers or parameters then extract them
   * here into an Authentication token that can be validated later. This implementation only
   * recognises password grant requests and extracts the username and password.
   *
  ...
  protected Authentication attemptTokenAuthentication(
    ...
    if (GRANT_TYPE_PASSWORD.equals(grantType)) {
      ...
      logger.debug(
          "Authentication credentials found password grant for '" + credentials.getName() + "'");
      authResult = authenticationManager.authenticate(credentials);

      if (authResult != null
          && authResult.isAuthenticated()
          && authResult instanceof UaaAuthentication) {
      ...
      if (assertion != null && samlAuthenticationFilter != null) {
        ...
        logger.debug(
            "No assertion or filter, not attempting SAML authentication for token endpoint.");
      ...
      if (assertion != null && externalOAuthAuthenticationManager != null) {
        ...
        ExternalOAuthCodeToken token =
            new ExternalOAuthCodeToken(null, null, null, assertion, null, null);
        token.setRequestContextPath(getContextPath(request));
        authResult = externalOAuthAuthenticationManager.authenticate(token);
      } else {
        logger.debug(
            "No assertion or authentication manager, not attempting JWT bearer authentication for token endpoint.");
