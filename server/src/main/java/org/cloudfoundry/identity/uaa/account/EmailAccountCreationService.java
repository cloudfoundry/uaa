...
public class EmailAccountCreationService implements AccountCreationService {
  ...
  public void beginActivation(String email, String password, String clientId, String redirectUri) {
    ...
    try {
      ...
      generateAndSendCode(
          ...
          scimUser.getId(),
          redirectUri,
          identityZoneManager.getCurrentIdentityZone());
    } catch (ScimResourceAlreadyExistsException e) {
      List<ScimUser> users =
          scimUserProvisioning.retrieveByUsernameAndOriginAndZone(
              email, OriginKeys.UAA, identityZoneManager.getCurrentIdentityZoneId());
      if (users.size() > 0) {
  ...
  private void generateAndSendCode(
    ...
    messageService.sendMessage(
        email, MessageType.CREATE_ACCOUNT_CONFIRMATION, subject, htmlContent);
  }

  @Override
  public AccountCreationResponse completeActivation(String code) {
    ExpiringCode expiringCode =
        codeStore.retrieveCode(code, identityZoneManager.getCurrentIdentityZoneId());
    if ((null == expiringCode)
        || ((null != expiringCode.getIntent())
            && !REGISTRATION.name().equals(expiringCode.getIntent()))) {
    ...
    ScimUser user =
        scimUserProvisioning.retrieve(
            data.get("user_id"), identityZoneManager.getCurrentIdentityZoneId());
    user =
        scimUserProvisioning.verifyUser(
            user.getId(), user.getVersion(), identityZoneManager.getCurrentIdentityZoneId());
    ...
    return new AccountCreationResponse(
        user.getId(), user.getUserName(), user.getUserName(), redirectLocation);
  }
    ...
    if (clientId != null) {
      try {
        ...
        String signupRedirectUrl =
            (String) clientDetails.getAdditionalInformation().get(SIGNUP_REDIRECT_URL);
        String matchingRedirectUri =
            findMatchingRedirectUri(
                registeredRedirectUris,
                redirectUri,
                Optional.ofNullable(signupRedirectUrl).orElse("home"));
        ...
        logger.debug(
            String.format(
                "Unable to find client with ID:%s for account activation redirect", clientId),
            nsce);
      }
    }
  ...
  public ScimUser createUser(String username, String password, String origin) {
    ...
    try {
      return scimUserProvisioning.createUser(
          scimUser, password, identityZoneManager.getCurrentIdentityZoneId());
  ...
  private String buildSubjectText() {
    ...
    boolean addBranding =
        StringUtils.hasText(companyName) && identityZoneManager.isCurrentZoneUaa();
    if (addBranding) {
  ...
  private String getEmailHtml(String code, String email, IdentityZone currentIdentityZone) {
    ...
    if (currentIdentityZone.isUaa()) {
      ctx.setVariable(
          "serviceName", StringUtils.hasText(companyName) ? companyName : "Cloud Foundry");
    } else {
      ctx.setVariable("serviceName", currentIdentityZone.getName());
    }
    ctx.setVariable(
        "servicePhrase",
        StringUtils.hasText(companyName) && currentIdentityZone.isUaa()
            ? companyName + " account"
            : "an account");
