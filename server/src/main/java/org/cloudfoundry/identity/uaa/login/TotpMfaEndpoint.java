...
public class TotpMfaEndpoint implements ApplicationEventPublisherAware {
  ...
  public UserGoogleMfaCredentials getUaaMfaCredentials() throws UaaPrincipalIsNotInSession {
    ...
    UserGoogleMfaCredentials result =
        mfaCredentialsProvisioning.getUserGoogleMfaCredentials(principal.getId());
    if (result == null) {
  ...
  public String generateQrUrl(
      Model model, @ModelAttribute("uaaMfaCredentials") UserGoogleMfaCredentials credentials)
    ...
    if (mfaCredentialsProvisioning.activeUserCredentialExists(
        uaaPrincipal.getId(), provider.getId())) {
      return "redirect:/login/mfa/verify";
    } else {
      String url =
          mfaCredentialsProvisioning.getOtpAuthURL(
              provider.getConfig().getIssuer(), credentials, uaaPrincipal.getName());
  ...
  public ModelAndView validateCode(
    ...
    if (!this.mfaPolicy.isAllowed(uaaPrincipal.getId()).isAllowed()) {
      throw new AuthenticationPolicyRejectionException(
    ...
    try {
      ...
      if (mfaCredentialsProvisioning.isValidCode(credentials, codeValue)) {
        ...
        publish(
            new MfaAuthenticationSuccessEvent(
                getUaaUser(uaaPrincipal),
                uaaAuth,
                getMfaProvider().getType().toValue(),
      ...
      publish(
          new MfaAuthenticationFailureEvent(
              getUaaUser(uaaPrincipal),
              uaaAuth,
              getMfaProvider().getType().toValue(),
              IdentityZoneHolder.getCurrentZoneId()));
      ...
      logger.debug(
          "Error validating the code for user: "
              + uaaPrincipal.getId()
              + ". Error: "
              + e.getMessage());
      publish(
  ...
  private ModelAndView renderEnterCodePage(Model model, UaaPrincipal uaaPrincipal) {
    model.addAttribute(
        "is_first_time_user", mfaCredentialsProvisioning.isFirstTimeMFAUser(uaaPrincipal));
    model.addAttribute("identity_zone", IdentityZoneHolder.get().getName());
