...
public class IdTokenGranter {
  ...
  private final List<String> GRANT_TYPES_THAT_MAY_GET_ID_TOKENS =
      Lists.newArrayList(
  ...
  public boolean shouldSendIdToken(
      String userId,
    ...
    if (clientScopes.stream().filter(Objects::nonNull).noneMatch(REQUIRED_OPENID_SCOPE::equals)) {
      return false;
    }
