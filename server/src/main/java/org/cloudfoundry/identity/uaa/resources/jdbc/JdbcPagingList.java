...
/**
 * List implementation backed by a database query, allowing iteration and sublist operations without
 * pulling the wole dataset into memory.
 *
 * <p>Not thread safe.
 *
 * @author Dave Syer
...
public class JdbcPagingList<E> extends AbstractList<E> {
  ...
  public JdbcPagingList(
      ...
      int pageSize) {
    this(
        new NamedParameterJdbcTemplate(jdbcTemplate), limitSqlAdapter, sql, args, mapper, pageSize);
  }
    ...
    if (current == null || index - start >= pageSize || index < start) {
      current =
          parameterJdbcTemplate.query(
              limitSqlAdapter.getLimitSql(sql, index, pageSize), args, mapper);
      start = index;
    }
  ...
  public List<E> subList(int fromIndex, int toIndex) {
    if (fromIndex < 0 || toIndex > size || fromIndex > toIndex) {
      throw new IndexOutOfBoundsException(
          "The indexes provided are outside the bounds of this list.");
    }
  ...
  /**
   * A list whose iterators are safe from changes in the underlying list. The size is not always
   * accurate if the underlying list changes, but the iterator will never say it has more elements
   * and then fail when iterated.
   *
   * <p>Not thread safe.
   *
