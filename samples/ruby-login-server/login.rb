...
class LoginApplication < Sinatra::Base
  ...
  login = lambda do
    ...
    unless username.nil? && password.nil?
      ...
      uaa_response = RestClient.post("#{UAA_TOKEN_SERVER}/login.do", \
  ...
  post '/confirm' do
    ...
    if choice == "yes"
      ...
      cookie = response.delete_cookie('uaa_cookie')
    end
  ...
  helpers do
    ...
    def post_to_authorize(request_params, headers)
      ...
      RestClient.post("#{UAA_TOKEN_SERVER}/oauth/authorize", \
                         request_params, \
                         headers) \
      {|response, request, result, &block| response}
    end
    ...
    def login_access_token
      ...
      login_response = RestClient.post("#{UAA_TOKEN_SERVER}/oauth/token", \
