...
pushd $(dirname $DIR)
  ...
  ./gradlew "-Dspring.profiles.active=$TESTENV" integrationTest --no-daemon --stacktrace --console=plain -x :cloudfoundry-identity-samples:assemble -x javadoc -x javadocJar
popd