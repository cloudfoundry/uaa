...
pushd $(dirname $DIR)
  ...
  ./gradlew "-Dspring.profiles.active=${TESTENV}" integrationTest jacocoAggregatedReport --no-daemon --stacktrace --console=plain -x :cloudfoundry-identity-samples:assemble
popd
