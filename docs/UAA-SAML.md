==================================================
User Account and Authentication SAML Integration
==================================================
...
through login.yml and the OAuth2 client configuration.

# Adding an Identity Provider
Adding an IdP involves changes in the IdP and changes in the UAA:
 * The IdP requires adding the UAA as relying party.
 * The UAA needs to add the IdP as trsuted identity provider

## Registration the UAA as relying party at the IdP
Registering SAML2 Identity Provider is product specific. In many products the steps are the following:
 * Import SAML2 Metadata. The UAA provides the metadata at https://\<identity-zone host\>/saml/metadata
 For the sample installations this is http://localhost:8080/uaa/saml/metadata
 * Configure supported authentication meths, supported NameId formats and other product specific settings
 
## Registering the IdP as trusted IdP
 UAA configures IdP by importing the SAML2 metadata. Depending on the identity zone, there are two approaches 
 for importing the metadata.
 * UAA (=default) identity zone: In this identity zone import the metadata by adding the metadata directly into the login.yml 
 configuration file. You can either include it as a URL, or by including the metadata file into the login.yml 
 configuration file.
 * Other identity zones
 
 
UAA supports different kind of identity providers. In the UAA they are labeled as ORIGIN:
 * Origin UAA: users stored in a local database
 * Origin LDAP: users stored in an LDAP directory
 * SAML with a custom Origin.

with the Lightweight Directory Access Protocol in two major areas. UAA integrates with the Lightweight Directory Access Protocol in two major areas. 
