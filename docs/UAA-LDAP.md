...
##### Restricting authentication to users members of a given group

With the "Search and Bind" and "Search and Compare" it is possible to restrict
the authentication to users that are members of a given group, if the LDAP object
contain the property `memberOf`.

To do so, we can tweak the `searchFilter` attribute to include a check of
the `memberOf` attribute:

```
searchFilter: '"(&(cn={0})(memberof=cn=cf,ou=groups,dc=test,dc=com))"'
```

In general this is true for any other attribute in the LDAP object of the user.

Limitations of this approach are:
 - Membership must be reflected as attributes in the user object (e.g. `memberOf`)
 - Transitive membership would not work.
