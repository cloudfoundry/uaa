...
public class JdbcScimGroupMembershipManager implements ScimGroupMembershipManager {
	...
	@Override
	public List<ScimGroupMember> removeMembers(final String groupId) throws ScimResourceNotFoundException {
	...
	private void validateMemberId(String id) {
		if (!StringUtils.hasText(id)) {
			throw new InvalidScimResourceException("member-id must be non-empty");
		}

		// check if it is an existing group or user id
		ScimCore member;
		try {
			member = scimUserProvisioning.retrieveUser(id);
			return;
		} catch (ScimResourceNotFoundException ex1) {
			try {
				member = scimGroupProvisioning.retrieveGroup(id);
			} catch (ScimResourceNotFoundException ex2) {
				throw new InvalidScimResourceException("Invalid group member - neither a group nor a user");
			}
		}
		return;
	}
