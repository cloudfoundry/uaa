...
public class JdbcScopeDelegationApprovalManager implements ScopeDelegationApprovalManager {
	...
	private static final String FIELDS = "user_id,client_id,scope,expires_at";

	private static final String ADD_AUTHZ_SQL = String.format("insert into %s ( %s ) values (?,?,?,?)", TABLE_NAME, FIELDS);

	private static final String REFRESH_AUTHZ_SQL = String.format("update %s set expiresAt=? where user_id=? and client_id=? and scope=?", TABLE_NAME);
	...
	private static final String EXPIRE_AUTHZ_SQL = String.format("update %s set expires_at = :expiry", TABLE_NAME);
