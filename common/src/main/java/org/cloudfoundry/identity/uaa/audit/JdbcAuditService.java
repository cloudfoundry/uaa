...
public class JdbcAuditService implements UaaAuditService {
	...
	public void secretChangeFailure(String message, Principal caller) {		
		createAuditRecord(caller.getName(), AuditEventType.SecretChangeFailure, getOrigin(caller), message);
	}
	
	@Override
	public void secretChangeFailure(String message, ClientDetails client, Principal caller) {
		createAuditRecord(client.getClientId(), AuditEventType.SecretChangeFailure, getOrigin(caller), message);
	}
