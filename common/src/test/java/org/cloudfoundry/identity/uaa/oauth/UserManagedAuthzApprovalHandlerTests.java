...
import org.cloudfoundry.identity.uaa.oauth.authz.JdbcApprovalsStore;
...
public class UserManagedAuthzApprovalHandlerTests {
	...
	public void setup()
	{
		...
		approvalsStore = new JdbcApprovalsStore(template);
		handler.setApprovalsStore(approvalsStore);
	}
	...
	public void testNoRequestedScopesButSomeApprovedScopes() {
		...
		approvalsStore.addApproval(new Approval(userAuthentication.getPrincipal(), "foo", "cloud_controller.read", nextWeek));
		approvalsStore.addApproval(new Approval(userAuthentication.getPrincipal(), "foo", "cloud_controller.write", nextWeek));
	...
	public void testRequestedScopesMatchApproval() {
		...
		approvalsStore.addApproval(new Approval(userAuthentication.getPrincipal(), "foo", "openid", nextWeek));
		approvalsStore.addApproval(new Approval(userAuthentication.getPrincipal(), "foo", "cloud_controller.read", nextWeek));
		approvalsStore.addApproval(new Approval(userAuthentication.getPrincipal(), "foo", "cloud_controller.write", nextWeek));
